...
def inference(x, is_training,
    ...
    for i, block_size in enumerate(num_blocks):
        with tf.variable_scope('scale' + str(i)):
            c['stack_stride'] = 1 if i == 0 else 2
            c['conv_filters_out'] = num_filters * 2**i
            c['num_blocks'] = num_blocks[i]
            x = stack(x, c)
    ...
    total_params = 0
    for var in all_vars:
        shape_list = var.get_shape()
        total_params += reduce((lambda x, y: x * y), shape_list) 
...
def block(x, c):
    ...
    conv_filters_out = c['conv_filters_out']
    if c['bottleneck']:
        ...
        with tf.variable_scope('b'):
            assert c['conv_filters_out'] == conv_filters_out
            c['ksize'] = 3
            c['stride'] = c['block_stride']
            c['conv_filters_out'] = conv_filters_out * c['width_factor']
            x = conv(x, c)
            x = bn(x, c)
            x = activation(x)
        ...
        with tf.variable_scope('c'):
            ...
            assert c['stride'] == 1
            c['ksize'] = 1
            c['conv_filters_out'] = conv_filters_out * 4
            x = conv(x, c)
            x = bn(x, c)
    else:
        with tf.variable_scope('A'):
    ...
    with tf.variable_scope('shortcut'):
        if conv_filters_out != filters_in or c['block_stride'] != 1:
            c['ksize'] = 1
            c['stride'] = c['block_stride']
            c['conv_filters_out'] = conv_filters_out * 4
