...
def attn(x, scope, n_state, *, past, params, train=False):
    ...
    with tf.variable_scope(scope):
        q = conv1d(x, 'c_attn_q', n_state, params=params)
        k = conv1d(x, 'c_attn_k', n_state, params=params)
        v = conv1d(x, 'c_attn_v', n_state, params=params)
...
def dropout(x, pdrop, train):
    # Temporily comment dropout due to RNG inst issue.
...
def model(X, params, labels=None, past=None, scope='model', reuse=False, train=False):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        output = tf.get_variable('output', [params["n_vocab"], params["n_embd"]], # Text encoding
        ...
        logits = tf.matmul(h_flat, output, transpose_b=True)
