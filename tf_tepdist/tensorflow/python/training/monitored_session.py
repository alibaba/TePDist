...
def MonitoredTrainingSession(
  ...
  if save_summaries_steps == USE_DEFAULT and save_summaries_secs == USE_DEFAULT:
    ...
    save_summaries_steps = None
  xla_flags = os.getenv("TF_XLA_FLAGS", None)
  control_flow_flag = os.getenv("TF_ENABLE_CONTROL_FLOW_V2", None)
  if (xla_flags and control_flow_flag == "1"):
    if "--tf_xla_cpu_global_jit=true" in xla_flags.split():
      save_summaries_steps = None
      save_summaries_secs = None
