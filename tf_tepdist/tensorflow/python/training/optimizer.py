...
def _annotate_all_global_vars():
  global_vars = ops.get_collection(ops.GraphKeys.GLOBAL_VARIABLES)
  for var in global_vars:
    op = var.op
    if not ops.OP_GROUP_STR in op.node_def.attr and \
       not ops.BACKWARD_STR in op.node_def.attr and \
       not var in ops.get_collection(ops.GraphKeys.TRAINABLE_VARIABLES):
      op._set_attr(ops.OP_GROUP_STR, attr_value_pb2.AttrValue(i=ops.GROUP_ID))
      op._set_attr(ops.BACKWARD_STR, attr_value_pb2.AttrValue(b=False))
      ops.GROUP_ID += 1
...
class Optimizer(
  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None):
    ...
    with ops.name_scope(name, self._name, skip_on_eager=False) as name:
      ...
      for grad, var, processor in converted_grads_and_vars:
        ...
        op_group_id = ops.trace_effective_group_id(grad.op)
        if op_group_id == 0:
          op_group_id = var.op.get_attr(ops.OP_GROUP_STR)
        else:
          var.op._set_attr(ops.OP_GROUP_STR, attr_value_pb2.AttrValue(i=op_group_id))

        with ops.name_scope(
            "update_" + scope_name,
            skip_on_eager=False), ops.colocate_with(var), \
            ops.dapple_scope(backward_flag=False, op_group=op_group_id):
  ...
  def get_slot(self, var, name):
    ...
    if hasattr(var, "_distributed_container"):
      ...
      res = mirrored_slot._get_closest()  # pylint: disable=protected-access
      ops.copy_op_group_annotation(var.op, res.op, backward=False)
      return res

    res = named_slots.get(_var_key(var), None)
    ops.copy_op_group_annotation(var.op, res.op, backward=False)
    return res
  ...
  def _create_non_slot_variable(self, initial_value, name, colocate_with):
    ...
    if v is None:
      ...
      with distribution_strategy.extended.colocate_vars_with(colocate_with):
        ...
        init_val_from_initializer = initializer(shape=[])
        v = variable_scope.variable(
            init_val_from_initializer, name=name, trainable=False,
