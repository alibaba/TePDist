...
BACKWARD_STR = "_BACKWARD"
OP_GROUP_STR = "_OP_GROUP"

GROUP_ID = 1
BACKWARD_FLAG = False
DAPPLE_SCOPE_MODE = False

class dapple_scope(object):
  def __init__(self, backward_flag, op_group):
    self._backward_flag = backward_flag
    self._op_group = op_group

  def __enter__(self):
    global GROUP_ID
    global BACKWARD_FLAG
    global DAPPLE_SCOPE_MODE
    GROUP_ID = self._op_group
    BACKWARD_FLAG = self._backward_flag
    DAPPLE_SCOPE_MODE = True
...
def trace_effective_group_id(op):
  """Effective group id resides in the non-control dependency ops.
     This function trace to the max group id if has multiple versions.
  ...
  while len(deque):
    op = deque.pop()
    if op.type == "Identity":
      ## Ignore Identity group id
      deque.append(op.inputs[0].op)
      continue

    if OP_GROUP_STR in op.node_def.attr:
      op_group_id = op.get_attr(OP_GROUP_STR)
      candidates.append(op_group_id)
      continue
...
def copy_op_group_annotation(primary, dst, backward=True):
  try:
    op_group_id = primary.get_attr(OP_GROUP_STR)
    dst._set_attr(OP_GROUP_STR, attr_value_pb2.AttrValue(i=op_group_id))
    dst._set_attr(BACKWARD_STR, attr_value_pb2.AttrValue(b=backward))
  except:
    pass
...
class RegisterGradient(object):
  ...
  def __call__(self, f):
    ...
    def wrapper(*args, **kwargs):
      ...
      for arg in args:
        op = arg.op if isinstance(arg, Tensor) else arg
        assert isinstance(op, Operation)
        if not OP_GROUP_STR in op.node_def.attr and not BACKWARD_STR in op.node_def.attr:
          op._set_attr(OP_GROUP_STR, attr_value_pb2.AttrValue(i=GROUP_ID))
          op._set_attr(BACKWARD_STR, attr_value_pb2.AttrValue(b=False))
          for src in op.inputs:
            if src.op.node_def.op == 'VarHandleOp':
              copy_op_group_annotation(op, src.op, backward=False)

      with dapple_scope(backward_flag = True, op_group = GROUP_ID):
        grads = f(*args, **kwargs)

      GROUP_ID += 1
      return grads

    _gradient_registry.register(wrapper, self._op_type)
    return wrapper
...
class Graph(object):
  ...
  def _create_op_internal(
    ...
    # Sharding scope start
    global GROUP_ID
    global BACKWARD_FLAG
    global DAPPLE_SCOPE_MODE

    if DAPPLE_SCOPE_MODE:
      ret._set_attr(OP_GROUP_STR, attr_value_pb2.AttrValue(i=GROUP_ID))
      ret._set_attr(BACKWARD_STR, attr_value_pb2.AttrValue(b=BACKWARD_FLAG))
    # Sharding scope end
