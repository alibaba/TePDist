...
def dropout_v2(x, rate, noise_shape=None, seed=None, name=None):
  ...
  with ops.name_scope(name, "dropout", [x]) as name:
    ...
    # NOTE: Stateless random is required in XLA compiler. We replace the
    # original random_ops with stateless_random_ops. Temporally, The seed is
    # a tuple with duplication values.
    # (TODO@siyu.wsy) Refine the random ops in dropout
    if not seed:
      seed = random.randint(0, 100)
    seed2 = seed
    seeds = math_ops.to_int32((seed, seed2))
    random_tensor = stateless_random_ops.stateless_random_uniform(
        shape=noise_shape, seed=seeds, dtype=x.dtype)
