...
from tensorflow.compiler.xla import rng_distribution_config_pb2
...
class Initializer(object):
  ...
  def __init__(self):
    self._attr_dict = {"distribution": "Constant",
                       "mean": 0.0, "stddev": 0.0,
                       "minval": 0.0, "maxval": 0.0,
                       "dtype": dtypes.float32, "seed": 0,
                       "const_val": 0.0}

  def extract_seed(self, tensor, distribution, dtype):
    if not distribution in ["random_uniform", "random_normal", "truncated_normal"]:
      return 0, 0
    rng = None
    if distribution == "random_uniform" and dtype.is_integer:
      rng = tensor.op
    else:
      rng = tensor.op.inputs[0].op.inputs[0].op

    seed = rng.get_attr("seed") if "seed" in rng.node_def.attr else 0
    seed2 = rng.get_attr("seed2") if "seed2" in rng.node_def.attr else 0
    return seed, seed2

  def apply_to_tensor(self, tensor):
    seed, seed2 = self.extract_seed(
        tensor, self._attr_dict["distribution"], self._attr_dict["dtype"])
    rng_conf_proto = rng_distribution_config_pb2.DistributionConfig(
        distribution=self._attr_dict["distribution"],
        mean=self._attr_dict["mean"], stddev=self._attr_dict["stddev"],
        minval=self._attr_dict["minval"], maxval=self._attr_dict["maxval"],
        dtype=self._attr_dict["dtype"].as_datatype_enum,
        seed=seed, seed2=seed2, const_val=self._attr_dict["const_val"])

    attr_value = attr_value_pb2.AttrValue(s=rng_conf_proto.SerializeToString())
    tensor.op._set_attr('_RNG_DIST_CONFIG', attr_value)
    return tensor
...
class Zeros(Initializer):
  ...
  def __call__(self, shape, dtype=None, partition_info=None):
    if dtype is None:
      dtype = self.dtype
    self._attr_dict["distribution"] = "Constant"
    self._attr_dict["const_val"] = 0.0
    self._attr_dict["dtype"] = dtype
    return self.apply_to_tensor(array_ops.zeros(shape, dtype))
...
class Ones(Initializer):
  ...
  def __call__(self, shape, dtype=None, partition_info=None):
    ...
    self._attr_dict["distribution"] = "Constant"
    self._attr_dict["const_val"] = 1.0
    self._attr_dict["dtype"] = dtype
    return self.apply_to_tensor(array_ops.ones(shape, dtype))
...
class Constant(Initializer):
  ...
  def __call__(self, shape, dtype=None, partition_info=None, verify_shape=None):
    ...
    self._attr_dict["distribution"] = "Constant"
    self._attr_dict["const_val"] = self.value
    self._attr_dict["dtype"] = dtype
    return self.apply_to_tensor(constant_op.constant_v1(
...
class RandomUniform(Initializer):
  ...
  def __call__(self, shape, dtype=None, partition_info=None):
    if dtype is None:
      dtype = self.dtype
    self._attr_dict["distribution"] = "random_uniform"
    self._attr_dict["minval"] = self.minval
    self._attr_dict["maxval"] = self.maxval
    self._attr_dict["dtype"] = dtype
    return self.apply_to_tensor(random_ops.random_uniform(
...
class RandomNormal(Initializer):
  ...
  def __call__(self, shape, dtype=None, partition_info=None):
    ...
    self._attr_dict["distribution"] = "random_normal"
    self._attr_dict["dtype"] = dtype
    self._attr_dict["mean"] = self.mean
    self._attr_dict["stddev"] = self.stddev
    return self.apply_to_tensor(random_ops.random_normal(
...
class TruncatedNormal(Initializer):
  ...
  def __call__(self, shape, dtype=None, partition_info=None):
    ...
    self._attr_dict["mean"] = self.mean
    self._attr_dict["stddev"] = self.stddev
    self._attr_dict["distribution"] = "truncated_normal"
    self._attr_dict["dtype"] = dtype
    return self.apply_to_tensor(random_ops.truncated_normal(
        shape, self.mean, self.stddev, dtype, seed=self.seed))
...
class UniformUnitScaling(Initializer):
  ...
  def __init__(self, factor=1.0, seed=None, dtype=dtypes.float32):
    super(UniformUnitScaling, self).__init__()
    self.factor = factor
    self.seed = seed
    self.dtype = _assert_float_dtype(dtypes.as_dtype(dtype))
  ...
  def __call__(self, shape, dtype=None, partition_info=None):
    ...
    self._attr_dict["distribution"] = "random_uniform"
    self._attr_dict["dtype"] = "dtype"
    self._attr_dict["minval"] = -max_val
    self._attr_dict["maxval"] = max_val
    return self.apply_to_tensor(random_ops.random_uniform(
...
class VarianceScaling(Initializer):
  ...
  def __call__(self, shape, dtype=None, partition_info=None):
    ...
    if self.distribution == "normal" or self.distribution == "truncated_normal":
      ...
      self._attr_dict["distribution"] = "truncated_normal"
      self._attr_dict["mean"] = 0.0
      self._attr_dict["stddev"] = stddev
      self._attr_dict["dtype"] = dtype
      return self.apply_to_tensor(random_ops.truncated_normal(
      ...
      self._attr_dict["distribution"] = "random_uniform"
      self._attr_dict["minval"] = -limit
      self._attr_dict["maxval"] = limit
      self._attr_dict["dtype"] = dtype
      return self.apply_to_tensor(random_ops.random_uniform(
