...
void ConstantFolding::ReplaceOperationWithSnapshot(
  ...
  const DataType dtype = GetDataTypeFromNodeOrProps(*node, properties);
  string rng_bytes;
  if (HasNodeAttr(*node, kRngDistConfig)) {
    GetNodeAttr(*node, kRngDistConfig, &rng_bytes);
  }
  if (dtype == DT_INVALID) return;
  ...
  node->clear_attr();
  if (!rng_bytes.empty()) {
    AddNodeAttr(kRngDistConfig, rng_bytes, node);
  }
  (*node->mutable_attr())["T"].set_type(dtype);
...
Status ConstantFolding::SimplifyArithmeticOperations(
    const GraphProperties& properties, bool use_shape_info,
    GraphDef* optimized_graph, NodeDef* node) {
  // Do not simplify the node with rng_dist_config
  if (HasNodeAttr(*node, kRngDistConfig)) return Status::OK();
