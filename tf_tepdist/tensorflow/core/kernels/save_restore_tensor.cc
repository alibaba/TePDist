...
Status RestoreTensorsV2(OpKernelContext* context, const Tensor& prefix,
  ...
  for (const size_t i : sorted_name_idx) {
    ...
    // Skip reading non-existent tensor
    if (!default_reader.Contains(tensor_name)) continue;
  ...
  for (auto i : sorted_name_idx) {
    ...
    // Allocate dummy placeholder tensor for output
    if (!default_reader.Contains(tensor_name)) {
      Tensor* dummy_tensor;
      TensorShape dummy_shape({0});
      context->allocate_output(i, dummy_shape, &dummy_tensor);
      continue;
    }
    auto op =
