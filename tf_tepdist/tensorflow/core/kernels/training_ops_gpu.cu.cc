...
template <typename T>
struct ApplyAdamWeightDecay<GPUDevice, T> {
  void operator()(const GPUDevice& d, typename TTypes<T>::Flat var,
                  typename TTypes<T>::Flat m, typename TTypes<T>::Flat v,
                  typename TTypes<T>::ConstScalar beta1_power,
                  typename TTypes<T>::ConstScalar beta2_power,
    ...
    if (use_weight_decay) {
      var.device(d) -= (lr * (beta2_power.constant(one) - beta2_power).sqrt() /
                       ...
                       + weight_decay_ratio.reshape(single).broadcast(bcast) *
                         var);
    } else {
      var.device(d) -= (lr * (beta2_power.constant(one) - beta2_power).sqrt() /
                        (beta1_power.constant(one) - beta1_power))
...
template struct functor::ApplyAdamWeightDecay<GPUDevice, Eigen::half>;
template struct functor::ApplyAdamWeightDecay<GPUDevice, float>;
template struct functor::ApplyAdamWeightDecay<GPUDevice, double>;
