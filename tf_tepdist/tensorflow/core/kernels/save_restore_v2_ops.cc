...
constexpr char kGlobalStep[] = "global_step";
namespace {
...
class SaveV2 : public OpKernel {
 public:
  explicit SaveV2(OpKernelConstruction* context) : OpKernel(context) {
    if (context->HasAttr("_max_to_keep")) {
      OP_REQUIRES_OK(context, context->GetAttr("_max_to_keep", &max_to_keep_));
    } else {
      max_to_keep_ = -1;
    }

    if (context->HasAttr("_remote_rpc")) {
      OP_REQUIRES_OK(context, context->GetAttr("_remote_rpc", &rpc_call_));
    } else {
      rpc_call_ = false;
    }
  }
  ...
  void Compute(OpKernelContext* context) override {
    ...
    for (int i = 0; i < num_tensors; ++i) {
      ...
      // Skip Saving empty tensor
      if (!tensor.NumElements()) continue;
      if (tensor_name == kGlobalStep && tensor.dtype() == DataType::DT_INT64) {
        global_step = *tensor.flat<int64>().data();
      }
    ...
    // Call RPC to do save on remote servers.
    if (rpc_call_ && global_step >= 0 && max_to_keep_ > 0) {
      auto platform_id = se::host::kHostPlatformId;
      auto rpc_client = xla::ClientLibrary::GetRPCClient(platform_id).ConsumeValueOrDie();
      OP_REQUIRES_OK(context, rpc_client->DoRemoteSave(max_to_keep_, global_step));
    }
  }
 private:
  int64 max_to_keep_;
...
class RestoreV2 : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const int num_tensors = static_cast<int>(tensor_names.NumElements());
    for (int i = 0; i < num_tensors; ++i) {
      auto& name = tensor_names_flat(i);
      if (name == kGlobalStep) {
        g_idx = i;
        break;
      }
    }
    if (rpc_call_ && g_idx > 0) {
      int64 global_step = *context->mutable_output(g_idx)->flat<int64>().data();
      VLOG(1) << "Restore global_step = " << global_step;
      auto platform_id = se::host::kHostPlatformId;
      auto rpc_client = xla::ClientLibrary::GetRPCClient(platform_id).ConsumeValueOrDie();
      OP_REQUIRES_OK(context, rpc_client->DoRemoteRestore(global_step));
    }
  }
