...
const char kOpGroup[] = "_OP_GROUP";
const char kNeedFetch[] = "_need_fetch";
...
VarHandleOp::VarHandleOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  bool backward = false;
  if (context->HasAttr(kBackward)) {
    OP_REQUIRES_OK(context, context->GetAttr(kBackward, &backward));
  }
  int op_group = 0;
  if (context->HasAttr(kOpGroup)) {
    OP_REQUIRES_OK(context, context->GetAttr(kOpGroup, &op_group));
  }
  bool need_fetch = false;
  if (context->HasAttr(kNeedFetch)) {
    OP_REQUIRES_OK(context, context->GetAttr(kNeedFetch, &need_fetch));
  }
  ...
  if (!is_anonymous_) {
    ...
    resource_.scalar<ResourceHandle>()() = MakeResourceHandle<Var>(
        ...
        allowed_devices_);
    resource_.scalar<ResourceHandle>()().set_backward(backward);
    resource_.scalar<ResourceHandle>()().set_op_group(op_group);
    resource_.scalar<ResourceHandle>()().set_need_fetch(need_fetch);
  }
}
...
class AssignVariableOp : public OpKernel {
 public:
  explicit AssignVariableOp(OpKernelConstruction* c) : OpKernel(c) {
    OP_REQUIRES_OK(c, c->GetAttr("dtype", &dtype_));
    if (c->HasAttr("_shape")) {
      OP_REQUIRES_OK(c, c->GetAttr("_shape", &var_shape_));
    }
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (!variable->is_initialized) {
      ++variable->already_init_ref_count;
      VLOG(2) << "[AssignVariableOp] variable->already_init_ref_count = "
              << variable->already_init_ref_count
              << ", handle.init_ref_count() = " << handle.init_ref_count();

      variable->is_initialized = (variable->already_init_ref_count ==
                                  handle.init_ref_count());
    }
    variable->set_need_fetch(handle.need_fetch());
    variable->set_backward(handle.backward());
    variable->set_op_group(handle.op_group());
    *variable->mutable_initializer_shape() = var_shape_;
  }

 private:
  DataType dtype_;
  TensorShape var_shape_;
...
template <typename Device, typename T>
class RegisterInitializationOp : public OpKernel {
 public:
  explicit RegisterInitializationOp(OpKernelConstruction* c) : OpKernel(c) {
    OP_REQUIRES_OK(c, c->GetAttr("dtype", &dtype_));
    OP_REQUIRES_OK(c, c->GetAttr("shape", &var_shape_));
    if (!c->GetAttr("_grappler_relax_allocator_constraints",
                    &relax_constraints_)
             .ok()) {
      relax_constraints_ = false;
  ...
  void Compute(OpKernelContext* context) override {
    ...
    mutex_lock ml(*variable->mu());
    *variable->mutable_initializer_string() = value.flat<tstring>()(0);
    if (!variable->is_initialized) {
      ++variable->already_init_ref_count;
      VLOG(2) << "[RegisterInitializationOp] variable->already_init_ref_count = "
              << variable->already_init_ref_count
              << ", handle.init_ref_count() = " << handle.init_ref_count();
      variable->is_initialized = (variable->already_init_ref_count ==
                                  handle.init_ref_count());
...
template <typename Device>
class RegisterInitializationOp<Device, Variant> : public OpKernel {
 public:
  explicit RegisterInitializationOp(OpKernelConstruction* c) : OpKernel(c) {
    OP_REQUIRES_OK(c, c->GetAttr("dtype", &dtype_));
    OP_REQUIRES_OK(c, c->GetAttr("shape", &var_shape_));
...
#define REGISTER_KERNELS(type)                                \
  REGISTER_KERNEL_BUILDER(Name("RegisterInitialization")            \
                              .Device(DEVICE_CPU)             \
                              .TypeConstraint<type>("dtype"), \
                          RegisterInitializationOp<Eigen::ThreadPoolDevice, type>);
...
#define REGISTER_GPU_KERNELS(type)                           \
  REGISTER_KERNEL_BUILDER(Name("RegisterInitialization")           \
                              ...
                              .TypeConstraint<type>("dtype") \
                              .HostMemory("resource"),       \
                          RegisterInitializationOp<GPUDevice, type>);
