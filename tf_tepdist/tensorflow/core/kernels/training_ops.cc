...
template <typename Device, typename T>
struct ApplyAdamWeightDecayNonCuda {
  void operator()(const Device& d, typename TTypes<T>::Flat var,
                  typename TTypes<T>::Flat m, typename TTypes<T>::Flat v,
                  typename TTypes<T>::ConstScalar beta1_power,
                  typename TTypes<T>::ConstScalar beta2_power,
                  ...
                  typename TTypes<T>::ConstScalar epsilon,
                  typename TTypes<T>::ConstFlat grad,
                  typename TTypes<T>::ConstScalar weight_decay_ratio,
    ...
    auto shard = [this, var_ptr, m_ptr, v_ptr, g_ptr, alpha, beta1, beta2,
                  use_weight_decay, weight_decay_ratio, epsilon,
      ...
      v += (g.square() - v) * (T(1) - beta2());
      if (use_weight_decay) {
        var -= alpha * (m / (v.sqrt() + epsilon()) + weight_decay_ratio() * var);
      } else {
        var -= (m * alpha) / (v.sqrt() + epsilon());
      }
    };
...
template <typename T>
struct ApplyAdamWeightDecay<CPUDevice, T>
    : ApplyAdamWeightDecayNonCuda<CPUDevice, T> {};
...
template <typename Device, typename T>
class ApplyAdamWeightDecayOp : public OpKernel {
 public:
  explicit ApplyAdamWeightDecayOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
    OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
    OP_REQUIRES_OK(ctx, ctx->GetAttr("use_weight_decay", &use_weight_decay_));
  }
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    const Tensor& weight_decay_rate = ctx->input(10);

    const Device& device = ctx->template eigen_device<Device>();
    functor::ApplyAdamWeightDecay<Device, T>()(
        device, var.flat<T>(), m.flat<T>(), v.flat<T>(),
        beta1_power.scalar<T>(), beta2_power.scalar<T>(), lr.scalar<T>(),
        beta1.scalar<T>(), beta2.scalar<T>(), epsilon.scalar<T>(),
        grad.flat<T>(), weight_decay_rate.scalar<T>(), use_weight_decay_);
...
#define REGISTER_KERNELS(D, T)                                     \
  REGISTER_KERNEL_BUILDER(                                         \
      Name("ApplyAdamWeightDecay").Device(DEVICE_##D).TypeConstraint<T>("T"), \
      ApplyAdamWeightDecayOp<D##Device, T>);                                  \
  REGISTER_KERNEL_BUILDER(Name("ResourceApplyAdamWeightDecay")     \
                              .HostMemory("var")                   \
                              .HostMemory("m")                     \
                              .HostMemory("v")                     \
                              .Device(DEVICE_##D)                  \
...
#define DECLARE_GPU_SPEC(T)                                   \
  template <>                                                 \
  void ApplyAdamWeightDecay<GPUDevice, T>::operator()(        \
      const GPUDevice& d, typename TTypes<T>::Flat var,       \
      typename TTypes<T>::Flat m, typename TTypes<T>::Flat v, \
      typename TTypes<T>::ConstScalar beta1_power,            \
      typename TTypes<T>::ConstScalar beta2_power,            \
      ...
      typename TTypes<T>::ConstScalar epsilon,                \
      typename TTypes<T>::ConstFlat grad,                     \
      typename TTypes<T>::ConstScalar weight_decay_rate,      \
      bool use_weight_decay); \
  extern template struct ApplyAdamWeightDecay<GPUDevice, T>;
DECLARE_GPU_SPEC(Eigen::half);
DECLARE_GPU_SPEC(float);
DECLARE_GPU_SPEC(double);
#undef DECLARE_GPU_SPEC
