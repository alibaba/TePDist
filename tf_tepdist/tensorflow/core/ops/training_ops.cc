...
template <bool is_sparse, bool is_resource>
static Status ApplyAdamWeightDecayShapeFn(InferenceContext* c) {
  ShapeHandle unused;
  ShapeHandle s = ShapeOrHandleShape<is_resource>(c, 0);                       // var
  TF_RETURN_IF_ERROR(c->Merge(s, ShapeOrHandleShape<is_resource>(c, 1), &s));  // m
  TF_RETURN_IF_ERROR(c->Merge(s, ShapeOrHandleShape<is_resource>(c, 2), &s));  // v
  TF_RETURN_IF_ERROR(c->WithRank(c->input(3), 0, &unused));       // beta1_power
  TF_RETURN_IF_ERROR(c->WithRank(c->input(4), 0, &unused));       // beta2_power
  TF_RETURN_IF_ERROR(c->WithRank(c->input(5), 0, &unused));       // lr
  TF_RETURN_IF_ERROR(c->WithRank(c->input(6), 0, &unused));       // beta1
  ...
  TF_RETURN_IF_ERROR(c->WithRank(c->input(8), 0, &unused));       // epsilon
  TF_RETURN_IF_ERROR(
      HandleGradAndIndicesInputs</*is_sparse=*/is_sparse, is_resource>(
          c, 9 /* grad_idx */, &s));
    //   HandleGradAndIndicesInputs</*is_sparse=*/false, is_resource>(
    //       c, sparse, 9 /* grad_idx */, &s));
  TF_RETURN_IF_ERROR(c->WithRank(c->input(10), 0, &unused));       // weight_decay_rate
  if (c->num_outputs() > 0) {
    c->set_output(0, s);
  }
  return Status::OK();
...
REGISTER_OP("ApplyAdamWeightDecay")
    .Input("var: Ref(T)")
    .Input("m: Ref(T)")
    .Input("v: Ref(T)")
    .Input("beta1_power: T")
    ...
    .Attr("T: numbertype")
    .Attr("use_locking: bool = false")
    .Attr("use_weight_decay: bool = false")
    .SetShapeFn(ApplyAdamWeightDecayShapeFn</*is_sparse=*/false, /*is_resource=*/false>);
    // .SetShapeFn([](InferenceContext* c) {
    //   return ApplyAdamWeightDecayShapeFn(c, false /* sparse */);
    // });

REGISTER_OP("ResourceApplyAdamWeightDecay")
    .Input("var: resource")
    .Input("m: resource")
    .Input("v: resource")
    .Input("beta1_power: T")
    ...
    .Attr("use_locking: bool = false")
    .Attr("use_weight_decay: bool = false")
    .SetShapeFn(ApplyAdamWeightDecayShapeFn</*is_sparse=*/false,/*is_resource=*/true>);
    // .SetShapeFn([](InferenceContext* c) {
    //   return ApplyAdamWeightDecayShapeFn(c, false /* sparse */);
    // });
