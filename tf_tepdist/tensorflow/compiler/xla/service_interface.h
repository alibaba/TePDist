...
class ServiceInterface {
 public:
  ...
  virtual Status TransferToServerHost(const TransferToServerRequest* arg,
                                  TransferToServerResponse* result) = 0;

  virtual Status DoRemoteSave(const DoRemoteSaveRequest* arg,
                              DoRemoteSaveResponse* result) = 0;

  virtual Status DoRemoteRestore(const DoRemoteRestoreRequest* arg,
                                 DoRemoteRestoreResponse* result) = 0;
  ...
  virtual Status ExecutePlan(const ExecutePlanRequest* arg,
                         ExecutePlanResponse* result) = 0;

  virtual Status BuildExecutionPlan(const BuildExecutionPlanRequest* arg,
                         BuildExecutionPlanResponse* result) = 0;

  virtual Status TransferModuleAndDefCtx(
      const TransferModuleAndDefCtxRequest* request,
      TransferModuleAndDefCtxResponse* response) = 0;

  virtual Status InitRemoteNcclComm(
      const InitRemoteNcclCommRequest* request,
      InitRemoteNcclCommResponse* response) = 0;

  virtual Status DispatchPlan(
      const DispatchPlanRequest* request,
      DispatchPlanResponse* response) = 0;

  virtual Status ExecuteRemotePlan(
      const ExecuteRemotePlanRequest* request,
      ExecuteRemotePlanResponse* response) = 0;

  virtual Status TransferHostRawData(
      const TransferHostRawDataRequest* request,
      TransferHostRawDataResponse* response) = 0;

  virtual Status TransferVarArgMap(
      const TransferVarArgMapRequest* request,
      TransferVarArgMapResponse* response) = 0;

  virtual Status FetchResourceVars(const FetchResourceVarsRequest* arg,
                                   FetchResourceVarsResponse* result) = 0;
