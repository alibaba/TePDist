...
XlaOp XlaBuilder::DAPPLEAllToAll(XlaOp operand, int64 split_dimension) {
  return ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    TF_ASSIGN_OR_RETURN(const Shape* operand_shape, GetShapePtr(operand));
    HloInstructionProto instr;
    *instr.mutable_shape() = operand_shape->ToProto();
    return AddInstruction(std::move(instr), HloOpcode::kDAPPLEAllToAll,
                          {operand});
  });
}
...
XlaOp DAPPLEAllToAll(const XlaOp operand, int64 split_dimension) {
  return operand.builder()->DAPPLEAllToAll(operand, split_dimension);
}
