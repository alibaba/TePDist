...
LocalExecutable::RunHelper(const absl::Span<const Shape* const> argument_shapes,
  ...
  auto entry = executable_->module().entry_computation();
  for (int i = 0; i < argument_shapes.size(); ++i) {
    if (!computation_layout.parameter_layout(i).MatchesLayoutInShape(
      ...
      if (param->dist_spec().get_dim_spec(0)->partition_dim() < 0) {
...
LocalClient::BuildDefModules(
    std::vector<std::pair<HloModule::DefContext*,
                          std::unique_ptr<HloModule>>> def_module_pairs,
    const ExecutableBuildOptions& build_options) {
  return local_service_->BuildDefModules(std::move(def_module_pairs),
                                         build_options);
}

// Facilitate cluster level distribution execution
StatusOr<std::vector<std::pair<HloModule::DefContext*,
                     std::unique_ptr<HloModule>>>>
LocalClient::PreBuildDefModuleForTaskDAG(
    const XlaComputation& computation,
    const absl::Span<const Shape* const> argument_layouts,
    const ExecutableBuildOptions& options) {
  ExecutableBuildOptions updated_options = options;
  ...
  return local_service_->PreBuildDefModuleForTaskDAG(
             computation, argument_layouts, updated_options);
}
