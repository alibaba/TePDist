...
/* static */ StatusOr<Client*> ClientLibrary::GetRPCClient(
  ...
  return it->second->rpc_client.get();
}
...
/* static */ StatusOr<Client*> ClientLibrary::GetRPCClient(
  ...
  ClientLibrary& client_library = Singleton();
  tensorflow::mutex_lock lock(client_library.service_mutex_);
  auto it = client_library.local_instances_.find(id);
  CHECK(it != client_library.local_instances_.end());
  return it->second->rpc_client.get();
}
...
/* static */ StatusOr<LocalClient*> ClientLibrary::GetOrCreateLocalClient(
  ...
  // The port the Server side is listening on.
  int64 port = -1;
  std::string ip_address;
  auto port_s = tensorflow::ReadInt64FromEnvVar("SERVER_PORT", -1,
                                                 &port);
  auto ip_s = tensorflow::ReadStringFromEnvVar("SERVER_IP", "localhost",
                                                &ip_address);
  if (!port_s.ok() || !ip_s.ok()) {
    return tensorflow::errors::InvalidArgument(
        "Failed to parse SERVER_PORT or SERVER_IP for client/server RPC connection...");
  }
  if (port > 0) {
    VLOG(0) << "RPC Client/Server mode, " << ip_address << ":" << port;
    ::grpc::ChannelArguments args;
    args.SetInt(GRPC_ARG_MAX_MESSAGE_LENGTH, std::numeric_limits<int32>::max());
    auto channel = ::grpc::CreateCustomChannel(
        absl::StrFormat("%s:%d", ip_address, port),
        ::grpc::InsecureChannelCredentials(), args);
    
    if (!channel->WaitForConnected(gpr_time_add(
        gpr_now(GPR_CLOCK_REALTIME), gpr_time_from_seconds(10, GPR_TIMESPAN)))) {
      return tensorflow::errors::FailedPrecondition(
          "Failed to connect to gRPC channel at ", ip_address, ":", port,
          " within a timeout of 10s.");
    }
    VLOG(0) << "Channel to server is connected on " << ip_address << ":" << port;
    instance->xla_service = grpc::XlaService::NewStub(channel);
    instance->stub.reset(new GRPCStub(instance->xla_service.get()));
    instance->rpc_client.reset(new Client(instance->stub.get()));
  } else {
    VLOG(2) << "LocalClient/LocalService mode.";
  }
