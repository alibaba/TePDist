...
StatusOr<bool> LayoutAssignment::Run(HloModule* module) {
  ...
  const std::vector<std::unique_ptr<HloModule::DefContext>>& all_def_ctx = module->all_def_ctx();
  std::unordered_set<HloComputation*> has_def_ctx;
  for (auto& def_ctx_ptr : all_def_ctx) {
    HloComputation* comp = module->Def2Compute(def_ctx_ptr.get());
    ComputationLayout layout(comp->ComputeProgramShape(), /*ignore_layouts=*/false);
    InsertOrDie(&computation_layouts_, comp, layout);
    has_def_ctx.insert(comp);
  }
