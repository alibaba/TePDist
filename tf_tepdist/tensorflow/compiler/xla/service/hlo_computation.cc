...
Status HloComputation::ReplaceParameterWithDifferentShape(int64 param_no,
    std::unique_ptr<HloInstruction> instruction) {
  CHECK_GE(param_no, 0);
  CHECK_LT(param_no, param_instructions_.size());

  HloInstruction* old_param = param_instructions_[param_no];
  auto new_param = AddInstructionInternal(std::move(instruction));
  TF_RETURN_IF_ERROR(old_param->ReplaceAllUsesWithDifferentShape(new_param));

  param_instructions_[param_no] = new_param;
  TF_RETURN_IF_ERROR(ForceRemoveInstruction(old_param));
...
Status HloComputation::ReplaceInstructionNoShapeCheck(
                                          HloInstruction* old_instruction,
                                          HloInstruction* new_instruction) {
  return ReplaceInstruction(old_instruction, new_instruction, false);
}

Status HloComputation::ReplaceInstruction(HloInstruction* old_instruction,
                                          HloInstruction* new_instruction,
                                          bool check_shape) {
  TF_RET_CHECK((!check_shape) ||
  ...
  if (check_shape) {
    ...
    TF_RETURN_IF_ERROR(old_instruction->ReplaceAllUsesWithDifferentShape(new_instruction));
  }
...
std::unique_ptr<HloComputation> HloComputation::CloneWithReplacements(
  ...
  for (auto instr : postorder) {
    ...
    new_instr->set_split_ordinal(instr->split_ordinal());
