...
struct CheckpointOptions {
  int64 global_step = -1;
  int64 max_to_keep = -1;
  bool lazy_save_ckpt = false;
  bool restore_from_ckpt = false;
};
...
class Service : public ServiceInterface {
 public:
  ...
  static StatusOr<std::unique_ptr<Service>> NewService(
      se::Platform* platform = nullptr, int task_index=0);
  static StatusOr<std::unique_ptr<Service>> NewService(
      const ServiceOptions& options);

  static VirtualClient* GetVirtualClient();

  // Task_index per node.
  // Here we use static variable of Service class as workaround for cycling of
  ...
  void BuildRunCost(HloModule::DefContext* def_ctx, Executable* exe);
  ...
  Status FixModuleForCompat(HloModule& module);

  Status ExecutePlan(const ExecutePlanRequest* arg,
                           ExecutePlanResponse* result) override;

  std::shared_ptr<LocalPlan> BuildDistPlan(
      const XlaComputation& computation,
      const absl::Span<const Shape* const>& argument_layouts,
      const ExecutableBuildOptions& options);

  Status BuildExecutionPlan(const BuildExecutionPlanRequest* arg, 
                            BuildExecutionPlanResponse* result) override;

  Status TransferModuleAndDefCtx(
      const TransferModuleAndDefCtxRequest* request,
      TransferModuleAndDefCtxResponse* response) override;

  Status InitRemoteNcclComm(
      const InitRemoteNcclCommRequest* request,
      InitRemoteNcclCommResponse* response) override;

  Status DispatchPlan(
      const DispatchPlanRequest* request,
      DispatchPlanResponse* response) override;

  Status ExecuteRemotePlan(
      const ExecuteRemotePlanRequest* request,
      ExecuteRemotePlanResponse* response) override;

  Status TransferHostRawData(
      const TransferHostRawDataRequest* request,
      TransferHostRawDataResponse* response) override;

  Status TransferVarArgMap(
      const TransferVarArgMapRequest* request,
      TransferVarArgMapResponse* response) override;

  Status FetchResourceVars(const FetchResourceVarsRequest* arg,
                           FetchResourceVarsResponse* result) override;

  StatusOr<XlaComputation> PreCompilationModuleFix(
  ...
  Status TransferToServerHost(const TransferToServerRequest* arg,
                          TransferToServerResponse* result) override;

  Status DoRemoteSave(const DoRemoteSaveRequest* arg,
                      DoRemoteSaveResponse* result) override;

  Status DoRemoteRestore(const DoRemoteRestoreRequest* arg,
                         DoRemoteRestoreResponse* result) override;
 ...
 protected:
  ...
  //StatusOr<std::vector<std::vector<const ShapedBuffer*>>>
  ResolveArguments(absl::Span<const GlobalDataHandle* const> arguments) const;

  StatusOr<std::vector<DAPPLEBuffer*>>
  ResolveDAPPLEBuffers(absl::Span<const GlobalDataHandle* const> d_buf_args);
  ...
  StatusOr<std::vector<std::unique_ptr<Executable>>> BuildExecutable(
      const HloModuleProto& module_proto,
      std::unique_ptr<HloModuleConfig> module_config, Backend* backend,
      se::StreamExecutor* executor,
      se::DeviceMemoryAllocator* device_allocator = nullptr);

  StatusOr<std::unique_ptr<Executable>> CompileDef(
      Backend* backend, se::StreamExecutor* executor,
      se::DeviceMemoryAllocator* device_allocator,
      std::unique_ptr<HloModule> module);

  void CompileDefContext(
      Backend* backend, se::StreamExecutor* executor,
      se::DeviceMemoryAllocator* device_allocator,
  ...
  StatusOr<std::vector<std::pair<HloModule::DefContext*,
           std::unique_ptr<HloModule>>>> PreBuildDefModules(
      const HloModuleProto& module_proto,
      std::unique_ptr<HloModuleConfig> module_config, Backend* backend,
      se::StreamExecutor* executor,
      se::DeviceMemoryAllocator* device_allocator);
  ...
  StatusOr<GlobalDataHandle> RegisteredForVariable(
      const LiteralProto& literal_proto, int global_idx);

  StatusOr<GlobalDataHandle> RegisteredForVariable(
      const ShapeProto& shape_proto, int global_idx);

  void ResetCheckpointOptions();

  ServiceOptions options_;

  // Cache containing previously built LocalPlan.
  ExecutionPlanCache plan_cache_;
  ...
  // Mutex to guard execute_plan state.
  tensorflow::mutex execute_plan_mutex_;

  CheckpointOptions ckpt_opts_;
  tensorflow::mutex do_save_remote_mutex_;

  TF_DISALLOW_COPY_AND_ASSIGN(Service);
};

int Service::TASK_INDEX = 0;
