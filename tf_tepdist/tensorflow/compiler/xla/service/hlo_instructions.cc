...
HloDAPPLEAllReduceInstruction::HloDAPPLEAllReduceInstruction(
    const Shape& shape, absl::Span<HloInstruction* const> operands,
    int num_replicas, bool sharding, std::string reduction_type)
    : HloDAPPLECollectiveInstruction(HloOpcode::kDAPPLEAllReduce, shape),
      num_replicas_(num_replicas), sharding_(sharding),
      reduction_type_(std::move(reduction_type)) {
  for (auto operand : operands) {
    AppendOperand(operand);
  }
}

std::vector<string> HloDAPPLEAllReduceInstruction::ExtraAttributesToStringImpl(
    const HloPrintOptions& options) const {
  std::vector<string> result;
  if (sharding_) {
    result.push_back("num_shards=" + std::to_string(num_replicas_));
  } else {
    result.push_back("num_replicas=" + std::to_string(num_replicas_));
  }
  result.push_back(StrCat("reduction_type=\"", CEscape(reduction_type_), "\""));
  result.push_back("split_ordinal=" + std::to_string(split_ordinal()));
  return result;
}

HloInstructionProto HloDAPPLEAllReduceInstruction::ToProto() const {
  HloInstructionProto proto = HloInstruction::ToProto();
  proto.set_num_replicas(num_replicas_);
  proto.set_dapple_sharding(sharding_);
  proto.set_reduction_type(reduction_type_);
  proto.set_split_ordinal(split_ordinal());
  return proto;
}

std::unique_ptr<HloInstruction>
HloDAPPLEAllReduceInstruction::CloneWithNewOperandsImpl(
    const Shape& shape, absl::Span<HloInstruction* const> new_operands,
    HloCloneContext* /*context*/) const {
  std::unique_ptr<HloDAPPLEAllReduceInstruction> inst =
      absl::make_unique<HloDAPPLEAllReduceInstruction>(
          shape, new_operands, num_replicas(), sharding(), reduction_type());
  inst->set_split_ordinal(split_ordinal());
  return inst;
}

HloDAPPLEAllToAllInstruction::HloDAPPLEAllToAllInstruction(
    const Shape& shape, absl::Span<HloInstruction* const> operands,
    const int64 split_dimension)
    : HloDAPPLECollectiveInstruction(HloOpcode::kDAPPLEAllToAll, shape),
...
std::vector<string> HloDAPPLEAllToAllInstruction::ExtraAttributesToStringImpl(
    const HloPrintOptions& options) const {
  std::vector<string> result;
  result.push_back("split_dimension=" + std::to_string(split_dimension_));
  result.push_back("split_ordinal=" + std::to_string(split_ordinal()));
  return result;
}

HloInstructionProto HloDAPPLEAllToAllInstruction::ToProto() const {
  HloInstructionProto proto = HloInstruction::ToProto();
...
std::unique_ptr<HloInstruction>
HloDAPPLEAllToAllInstruction::CloneWithNewOperandsImpl(
    const Shape& shape, absl::Span<HloInstruction* const> new_operands,
    HloCloneContext* /*context*/) const {
  std::unique_ptr<HloDAPPLEAllToAllInstruction> inst =
      absl::make_unique<HloDAPPLEAllToAllInstruction>(
          shape, new_operands, split_dimension());
  inst->set_split_ordinal(split_ordinal());
  return inst;
}

HloDAPPLEAllGatherInstruction::HloDAPPLEAllGatherInstruction(
    const Shape& shape, absl::Span<HloInstruction* const> operands,
    int64 all_gather_dimension)
    : HloDAPPLECollectiveInstruction(HloOpcode::kDAPPLEAllGather, shape),
...
std::vector<string> HloDAPPLEAllGatherInstruction::ExtraAttributesToStringImpl(
    const HloPrintOptions& options) const {
  std::vector<string> result;
...
HloInstructionProto HloDAPPLEAllGatherInstruction::ToProto() const {
  HloInstructionProto proto = HloInstruction::ToProto();
...
std::unique_ptr<HloInstruction>
HloDAPPLEAllGatherInstruction::CloneWithNewOperandsImpl(
    const Shape& shape, absl::Span<HloInstruction* const> new_operands,
    HloCloneContext* /*context*/) const {
  std::unique_ptr<HloDAPPLEAllGatherInstruction> inst =
      absl::make_unique<HloDAPPLEAllGatherInstruction>(
          shape, new_operands, all_gather_dimension());
  inst->set_split_ordinal(split_ordinal());
  return inst;
}
...
string HloFusionInstruction::ToCategory() const {
  switch (fusion_kind()) {
    ...
    case FusionKind::kSubModuleCall:
      return "sub module call";
  }
}
...
HloCustomCollectiveInstruction::HloCustomCollectiveInstruction(
    const Shape& shape,  absl::Span<HloInstruction* const> operands,
    absl::string_view collective_type, string opaque, int split_ordinal)
    : HloDAPPLECollectiveInstruction(
          HloOpcode::kCustomCollective, shape, split_ordinal),
      collective_type_(collective_type.begin(), collective_type.end()) {
  set_raw_backend_config_string(std::move(opaque));
  for (auto operand : operands) {
    AppendOperand(operand);
  }
...
HloInstructionProto HloCustomCollectiveInstruction::ToProto() const {
  HloInstructionProto proto = HloInstruction::ToProto();
  proto.set_custom_call_target(collective_type_);
  return proto;
}

std::unique_ptr<HloInstruction>
HloCustomCollectiveInstruction::CloneWithNewOperandsImpl(
    const Shape& shape, absl::Span<HloInstruction* const> new_operands,
    HloCloneContext* context) const {
  auto cloned = absl::make_unique<HloCustomCollectiveInstruction>(
      shape, new_operands, collective_type(), opaque(), split_ordinal());
  return std::move(cloned);
}
