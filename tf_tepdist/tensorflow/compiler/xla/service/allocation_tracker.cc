...
StatusOr<GlobalDataHandle>
AllocationTracker::ProduceAndRegsiterDAPPLEBufferHandle() {
  tensorflow::mutex_lock lock(mutex_);
  VLOG(2) << "RegisterDAPPLEBufferHandle";
  int64 handle = next_handle_++;
  int64 var_handle = next_variable_handle_++;
  handle_to_registered_buffers_[handle].emplace_back(
     absl::make_unique<RegisteredBuffer>(var_handle));
  GlobalDataHandle result;
  result.set_handle(handle);
  result.set_variable(true);
  VLOG(2) << "handle: " << handle;
  return result; 
}

StatusOr<DAPPLEBufferHandle> AllocationTracker::ResolveDAPPLEBufferHandle(
    const GlobalDataHandle& data) const {
  tensorflow::mutex_lock lock(mutex_);
  VLOG(2) << "RegisterDAPPLEBufferHandle";
  auto it = handle_to_registered_buffers_.find(data.handle());
  if (it == handle_to_registered_buffers_.end()) {
    return NotFound("no allocation record for global data handle: %d",
                    data.handle());
  }

  return it->second[0]->dapple_buf_handle_;
}
...
StatusOr<GlobalDataHandle> AllocationTracker::RegisterInternal(
  ...
  for (auto& shaped_buffer : replicated_buffers) {
    ...
    handle_to_registered_buffers_[handle].emplace_back(
        absl::make_unique<RegisteredBuffer>(
            ReleaseIfScopedShapedBuffer(std::move(shaped_buffer))));
  }

  GlobalDataHandle result;
  result.set_handle(handle);
  VLOG(2) << "registered handle: " << handle;
  return result;
}
...
Status AllocationTracker::Unregister(const GlobalDataHandle& data) {
  ...
  for (auto& registered_buffer : it->second) {
    if (!registered_buffer->is_variable_) {
      registered_buffer->shaped_buffer_.reset();
    }
  }
  return Status::OK();
}
...
StatusOr<std::vector<const ShapedBuffer*>> AllocationTracker::ResolveInternal(
  ...
  for (const auto& registered_buffer : it->second) {
    if (registered_buffer == nullptr) {
      return InvalidArgument("global data handle %d was previously deallocated",
                             data.handle());
    }
    if (registered_buffer->is_variable_) {
      return InvalidArgument("global data handle %d represents variable",
                             data.handle());
    }
    replicated_buffers.push_back(registered_buffer->shaped_buffer_.get());
  }
...
RegisteredBuffer::RegisteredBuffer(ShapedBuffer shaped_buffer)
    : is_variable_(false) {
  shaped_buffer_ = absl::make_unique<ShapedBuffer>(
      std::move(shaped_buffer));
}

RegisteredBuffer::RegisteredBuffer(DAPPLEBufferHandle dapple_buf_handle)
    : dapple_buf_handle_(dapple_buf_handle), is_variable_(true) {}

RegisteredBuffer::RegisteredBuffer(RegisteredBuffer&& rb)
    : is_variable_(rb.is_variable_),
      shaped_buffer_(std::move(rb.shaped_buffer_)) {}
