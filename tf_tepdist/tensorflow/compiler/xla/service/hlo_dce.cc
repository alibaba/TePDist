...
StatusOr<bool> HloDCE::Run(HloModule* module) {
  ...
  std::unordered_set<HloComputation*> keep_set;
  for (auto& it : module->DefComputation()) {
    keep_set.insert(it.second);
  }
  for (auto* computation : module->MakeComputationPostOrder()) {
    if (!live_computations.contains(computation)) {
      // Do not remove .GA computations, because they are actually entry.
      if (!keep_set.count(computation)) {
