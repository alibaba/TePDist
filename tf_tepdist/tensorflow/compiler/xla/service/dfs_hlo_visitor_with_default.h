...
class DfsHloVisitorWithDefaultBase
 ...
 public:
  ...
  Status HandleDAPPLEAllReduce(HloInstructionPtr crs) override {
    return DefaultAction(crs);
  }
  Status HandleDAPPLEAllToAll(HloInstructionPtr crs) override {
    return DefaultAction(crs);
  }
  Status HandleAllToAll(HloInstructionPtr hlo) override {
    return DefaultAction(hlo);
  }
  Status HandleDAPPLEAllGather(HloInstructionPtr crs) override {
    return DefaultAction(crs);
  }
  Status HandleCustomCollective(HloInstructionPtr crs) override {
    return DefaultAction(crs);
  }
