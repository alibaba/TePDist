...
StatusOr<std::unique_ptr<HloInstruction>> HloInstruction::CreateFromProto(
  ...
  switch (opcode) {
    ...
    case HloOpcode::kDAPPLEAllReduce: {
      //CHECK(2 == all_operands().size());
      instruction = CreateDAPPLEAllReduce(
          shape, all_operands(), proto.num_replicas(), proto.dapple_sharding(),
          proto.reduction_type());
      auto* coll = DynCast<HloDAPPLECollectiveInstruction>(instruction.get());
      break;
    }

    case HloOpcode::kCustomCollective: {
      instruction = CreateCustomCollective(
          shape, all_operands(), proto.custom_call_target(), proto.backend_config());
      break;
    }

    case HloOpcode::kDAPPLEAllToAll: {
      CHECK(proto.dimensions_size() > 0);
      instruction = CreateDAPPLEAllToAll(
          shape, all_operands(), proto.dimensions(0));
      auto* coll = DynCast<HloDAPPLECollectiveInstruction>(instruction.get());
      break;
    }
    ...
    case HloOpcode::kDAPPLEAllGather: {
      TF_RET_CHECK(proto.dimensions_size() == 1)
          << "DAPPLEAllGather cannot have more than 1 all-gather dimensions";
      int64 all_gather_dimension = proto.dimensions(0);
      instruction = CreateDAPPLEAllGather(
          shape, all_operands(), all_gather_dimension);
      auto* coll = DynCast<HloDAPPLECollectiveInstruction>(instruction.get());
      coll->set_split_ordinal(proto.split_ordinal());
      break;
    }
  ...
  // Auto-Parallel Start
  if (proto.has_dist_spec_proto()) {
    DistSpec dist_spec = DistSpec::CreateFromProto(proto.dist_spec_proto());
    *(instruction->mutable_dist_spec()) = std::move(dist_spec);
  }
  instruction->set_split_ordinal(proto.split_ordinal());
  // Auto-Parallel End
...
HloInstruction::CreateDAPPLEAllReduce(
    const Shape& shape, absl::Span<HloInstruction* const> operands,
    int num_replicas, bool sharding, std::string reduction_type) {
  return absl::make_unique<HloDAPPLEAllReduceInstruction>(
      shape, operands, num_replicas, sharding, reduction_type);
}

/* static */ std::unique_ptr<HloInstruction>
HloInstruction::CreateCustomCollective(
    const Shape& shape, absl::Span<HloInstruction* const> operands,
    absl::string_view collective_type, string opaque) {
  return absl::make_unique<HloCustomCollectiveInstruction>(
      shape, operands, collective_type, opaque);
}

/* static */ std::unique_ptr<HloInstruction>
HloInstruction::CreateDAPPLEAllToAll(
    const Shape& shape, absl::Span<HloInstruction* const> operands,
    const int64 split_dimension) {
  return absl::make_unique<HloDAPPLEAllToAllInstruction>(
      shape, operands, split_dimension);
}
...
HloInstruction::CreateDAPPLEAllGather(
  ...
  return absl::make_unique<HloDAPPLEAllGatherInstruction>(
...
/* static */ std::unique_ptr<HloInstruction> HloInstruction::CreateReshapeNoShapeCheck(
    const Shape& shape, HloInstruction* operand, int64 inferred_dimension) {
  return absl::make_unique<HloReshapeInstruction>(shape, operand,
                                                  inferred_dimension);
}
...
std::unique_ptr<HloInstruction> HloInstruction::CloneWithNewOperands(
  ...
  *clone->mutable_dist_spec() = dist_spec();
  clone->set_split_ordinal(split_ordinal());
...
void HloInstruction::SortUsersAndControlSuccessorsById() {
  std::sort(control_successors_.begin(), control_successors_.end(),
  ...
  for (int64 index = 0; index < users_.size(); ++index) {
    user_map_[users_[index]] = index;
  }
}
...
// Auto-Parallel Start
bool HloInstruction::IsIdentity() const {
  switch (opcode_) {
    //case HloOpcode::kCopy:
    case HloOpcode::kConstant:
    case HloOpcode::kParameter:
...
string HloInstruction::ToStringWithCanonicalNameMap(
  ...
  string dist_spec_str = ", DistSpec=\"{" + dist_spec().ToString() + "}\"";
  StrAppend(&result, dist_spec_str);
...
HloInstructionProto HloInstruction::ToProto() const {
  ...
  *proto.mutable_frontend_attributes() = frontend_attributes_;
  ...
  // Auto-Parallel Start
  proto.set_split_ordinal(split_ordinal());
  dist_spec_.ToProto(*proto.mutable_dist_spec_proto());
  // Auto-Parallel End
  return proto;
}
...
Status HloInstruction::Visit(DfsHloVisitorBase<HloInstructionPtr>* visitor) {
  switch (opcode_) {
    ...
    case HloOpcode::kDAPPLEAllReduce:
      return visitor->HandleDAPPLEAllReduce(this);
    case HloOpcode::kDAPPLEAllToAll:
      return visitor->HandleDAPPLEAllToAll(this);
    case HloOpcode::kDAPPLEAllGather:
      return visitor->HandleDAPPLEAllGather(this);
    case HloOpcode::kCustomCollective:
      return visitor->HandleCustomCollective(this);
    case HloOpcode::kAllToAll:
...
string ToString(HloInstruction::FusionKind kind) {
  switch (kind) {
    ...
    case HloInstruction::FusionKind::kCustom:
      return "kCustom";
    case HloInstruction::FusionKind::kSubModuleCall:
      return "kSubModuleCall";
    default: CHECK(0); return string("");
  }
}
...
string OpMetadataToString(const OpMetadata& metadata) {
  ...
  result.push_back(StrCat("op_group=\"", metadata.op_group(), "\""));
  result.push_back(StrCat("backward=\"", metadata.backward(), "\""));
...
// Auto-Parallel Start
const DistSpec& HloInstruction::dist_spec() const { return dist_spec_; }
DistSpec* HloInstruction::mutable_dist_spec() { return &dist_spec_; }
void HloInstruction::set_dist_spec(const std::string dist_spec_str) {
  auto dist_spec_no_bracket = dist_spec_str.substr(1, dist_spec_str.size() - 2);
  VLOG(0) << dist_spec_no_bracket;

  std::vector<std::string> dist_specs =
      tensorflow::str_util::Split(dist_spec_no_bracket, ',');
  int64 stride = 0;
  int64 stride_on_dim = 0;
  int dim_to_slice = -1;
  int num_splits = 1;
  for (auto spec_val : dist_specs) {
    std::vector<std::string> pair = tensorflow::str_util::Split(spec_val, '=');
    CHECK(pair.size() == 2);
    auto spec = pair[0];
    if (spec == "stride") {
      stride = std::stoi(pair[1]);
    } else if (spec == "stride_on_dim") {
      stride_on_dim = std::stoi(pair[1]);
    } else if (spec == "dim_to_slice") {
      dim_to_slice = std::stoi(pair[1]);
    } else if (spec == "num_splits") {
      num_splits = std::stoi(pair[1]);
    } else if (spec == "stage") {
      // TODO: add this when needed.
    } else {
      CHECK(0 && "Unknown dist-spec") << spec;
    }
  }
  dist_spec_.get_dim_spec(0)->set_layout_aware_partition(
      stride, stride_on_dim, dim_to_slice, num_splits);
}

void HloInstruction::set_allow_eliminate(bool allow_eliminate) {
  allow_eliminate_ = allow_eliminate;
}

const bool HloInstruction::allow_eliminate() const {
  return allow_eliminate_;
}
// Auto-Parallel End
