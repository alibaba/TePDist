...
Status CombineAllReduces(absl::Span<HloInstruction* const> to_combine) {
  ...
  for (HloInstruction* hlo : to_combine) {
    ...
    TF_RET_CHECK(hlo->opcode() == HloOpcode::kDAPPLEAllReduce);
  ...
  auto hlo = to_combine[0];
  int num_replicas = Cast<HloDAPPLEAllReduceInstruction>(hlo)->num_replicas();
  int split_ordinal = Cast<HloDAPPLEAllReduceInstruction>(hlo)->split_ordinal();
  bool sharding = Cast<HloDAPPLEAllReduceInstruction>(hlo)->sharding();
  std::string reduction_type =
      Cast<HloDAPPLEAllReduceInstruction>(hlo)->reduction_type();
  combined = computation.AddInstruction(HloInstruction::CreateDAPPLEAllReduce(
      ShapeUtil::MakeTupleShape(operand_shapes), operands, num_replicas,
      sharding, reduction_type));
  DynCast<HloDAPPLECollectiveInstruction>(combined)
      ->set_split_ordinal(split_ordinal);
...
struct GroupKey {
  GroupKey(const HloInstruction* hlo, const HloDomainMap& domain_map) {
    CHECK(hlo->opcode() == HloOpcode::kDAPPLEAllReduce);
    opcode = HloOpcode::kAdd;
    accum_type = xla::F32; // to support BF16 future?
  }
...
StatusOr<InstructionGroups> CreateComputationGroups(
  ...
  for (HloInstruction* instruction : computation->MakeInstructionPostOrder()) {
    if (instruction->opcode() != HloOpcode::kDAPPLEAllReduce) {
      continue;
    }
...
DAPPLEAllReduceCombiner::DAPPLEAllReduceCombiner(
    int64 combine_threshold_in_bytes, int64 combine_threshold_count)
    : combine_threshold_in_bytes_(combine_threshold_in_bytes),
      combine_threshold_count_(combine_threshold_count) {}

StatusOr<bool> DAPPLEAllReduceCombiner::Run(HloModule* module) {
