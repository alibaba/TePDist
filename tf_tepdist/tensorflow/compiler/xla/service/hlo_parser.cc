...
bool HloParserImpl::ParseInstructionRhs(HloComputation::Builder* builder,
  ...
  optional<std::string> dist_spec;
  attrs["DistSpec"] = {/*required=*/false, AttrTy::kString, &dist_spec};
  ...
  switch (opcode) {
    ...
    case HloOpcode::kDAPPLEAllReduce: {
      optional<int32> num_replicas;
      optional<int32> num_shards;
      optional<std::string> reduction_type;
      bool sharding;
      int32 replica_or_shard_num;
      attrs["num_replicas"] = {/*required=*/false, AttrTy::kInt32,
                               &num_replicas};
      attrs["num_shards"] = {/*required=*/false, AttrTy::kInt32, &num_shards};
      attrs["reduction_type"] = {/*required=*/true, AttrTy::kString,
                                 &reduction_type};
      if (!ParseOperands(&operands) || !ParseAttributes(attrs)) {
        return false;
      }
      if (num_replicas) {
        replica_or_shard_num = *num_replicas;
        sharding = false;
      } else {
        CHECK(num_shards);
        replica_or_shard_num = *num_shards;
        sharding = true;
      }
      instruction = builder->AddInstruction(
          HloInstruction::CreateDAPPLEAllReduce(shape, operands,
              replica_or_shard_num, sharding, *reduction_type));
      break;
    }
    ...
    case HloOpcode::kDAPPLEAllGather: {
      ...
      instruction = builder->AddInstruction(
          HloInstruction::CreateDAPPLEAllGather(shape, operands,
	      dimensions->at(0)));
      break;
    }
    ...
    default:
      VLOG(0) << "Unhandled opcode for HloParserImpl::ParseInstructionRhs:"
              << HloOpcodeString(opcode);
      CHECK(0);
  }
  ...
  if (dist_spec) {
    instruction->set_dist_spec(std::move(*dist_spec));
  }
