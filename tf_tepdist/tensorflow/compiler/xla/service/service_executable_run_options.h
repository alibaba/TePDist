...
class ServiceExecutableRunOptions {
 public:
  ...
  const std::vector<int64>& task_id() const {
    return task_id_;
  }
  const std::shared_ptr<CommDevManager> comm_dev_mgr() const {
    return comm_dev_mgr_;
  }
  void set_task_id(const std::vector<int64>& task_id) {
    task_id_ = task_id;
  }
