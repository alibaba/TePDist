...
// Auto-Parallel Start
// NOTE: re-declaration of derivation ds
using Split = std::tuple<HloInstruction*, int>;
...
class HloInstruction {
 public:
  ...
  static std::unique_ptr<HloInstruction> CreateDAPPLEAllReduce(
      const Shape& shape, absl::Span<HloInstruction* const> operands,
      int num_replicas, bool sharding, std::string reduction_type);

  static std::unique_ptr<HloInstruction> CreateCustomCollective(
      const Shape& shape, absl::Span<HloInstruction* const> operands,
  ...
  static std::unique_ptr<HloInstruction> CreateDAPPLEAllToAll(
      const Shape& shape, absl::Span<HloInstruction* const> operands,
      const int64 split_dimension);

  static std::unique_ptr<HloInstruction> CreateDAPPLEAllGather(
      const Shape& shape, absl::Span<HloInstruction* const> operands,
  ...
  static std::unique_ptr<HloInstruction> CreateReshapeNoShapeCheck(
      const Shape& shape, HloInstruction* operand,
      int64 inferred_dimension = -1);
  ...
  // Auto-Parallel Start
  // For HybridParallel. We define the instruction which produces tensor without
  // any predecessor is Identity.
  ...
  OpMetadata& metadata() { return metadata_; }
  ...
  // Auto-Parallel Start
  // DistSpec getter (RO / RW)
  const DistSpec& dist_spec() const;
  void set_dist_spec(const std::string dist_spec_str);
  DistSpec* mutable_dist_spec();
  void set_allow_eliminate(bool allow_eliminate);
  const bool allow_eliminate() const;
  void SortUsersAndControlSuccessorsById();
  void set_split_ordinal(int split_ordinal) { split_ordinal_ = split_ordinal; }
  int split_ordinal() const { return split_ordinal_; }
  // Auto-Parallel End
 ...
 private:
  ...
  // Auto-Parallel Start
  //  HLO Distribution Specification
  DistSpec dist_spec_;
  // Indicates whether this instruction can be eliminated by hlo passes.
  // Only HloCSE read this flag currently.
  bool allow_eliminate_ = true;
  // Auto-Parallel End
...
// Rename community type definings for better understanding.
template <typename ValueT>
using HloMutableInstMap = std::map<HloInstruction*, ValueT, HloPtrComparator>;
template <typename ValueT>
using HloInstMap = std::map<const HloInstruction*, ValueT, HloPtrComparator>;
using HloMutableInstSet = std::set<HloInstruction*, HloPtrComparator>;
using HloInstSet = std::set<const HloInstruction*, HloPtrComparator>;
