...
class HloDAPPLECollectiveInstruction : public HloInstruction {
 public:
  explicit HloDAPPLECollectiveInstruction(
      HloOpcode opcode, const Shape& shape, int split_ordinal = 0)
  : HloInstruction(opcode, shape) { split_ordinal_ = split_ordinal; }
};

class HloDAPPLEAllReduceInstruction : public HloDAPPLECollectiveInstruction {
 public:
  explicit HloDAPPLEAllReduceInstruction(const Shape& shape, 
      absl::Span<HloInstruction* const> operands,
      int num_replicas, bool sharding, std::string reduction_type);
  
  int num_replicas() const { return num_replicas_; }
  bool sharding() const { return sharding_; }
  std::string reduction_type() const { return reduction_type_; }
 ...
 private:
  // N.B., when sharding_=true, num_replicas_ actually refers to num of shards.
  int num_replicas_;
  bool sharding_;
  // Reduction type, valid types are: Sum, Min and Max.
  std::string reduction_type_;
};

class HloDAPPLEAllToAllInstruction : public HloDAPPLECollectiveInstruction {
 public:
  explicit HloDAPPLEAllToAllInstruction(const Shape& shape,
      absl::Span<HloInstruction* const> operands,
  ...
  const int64 split_dimension() const { return split_dimension_; }
 ...
 private:
  // The dimension along which the operand is split
  int64 split_dimension_;
};

class HloDAPPLEAllGatherInstruction : public HloDAPPLECollectiveInstruction {
 public:
  explicit HloDAPPLEAllGatherInstruction(
...
class HloCustomCollectiveInstruction : public HloDAPPLECollectiveInstruction {
 public:
  HloCustomCollectiveInstruction(
      const Shape& shape, absl::Span<HloInstruction* const> operands,
      absl::string_view collective_type, string opaque, int split_ordinal = 0);

  const string& opaque() const { return raw_backend_config_string(); }
  const string& collective_type() const { return collective_type_; }
