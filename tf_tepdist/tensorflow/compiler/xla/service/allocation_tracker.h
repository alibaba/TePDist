...
class RegisteredBuffer {
 public:
  RegisteredBuffer(ShapedBuffer shaped_buffer);
  RegisteredBuffer(DAPPLEBufferHandle dapple_buf_handle);

  // Movable, but not copyable.
  RegisteredBuffer(RegisteredBuffer&& rb);
  RegisteredBuffer& operator=(RegisteredBuffer&&);
  RegisteredBuffer(const RegisteredBuffer&) = delete;
  RegisteredBuffer& operator=(const RegisteredBuffer&) = delete;

  DAPPLEBufferHandle dapple_buf_handle() const {
      return dapple_buf_handle_;
  }

  std::unique_ptr<ShapedBuffer> shaped_buffer_;
  // Currently, we work around for DAPPLEBuffer registration directly.
  // (TODO) Decouple DAPPLEBuffer with VirtualClient, then allow
  // DAPPLEBuffer registration in AllocationTracker.
  DAPPLEBufferHandle dapple_buf_handle_;
  bool is_variable_;
};
...
class AllocationTracker {
 public:
  ...
  // allocator.
  AllocationTracker(Backend* backend)
      : backend_(backend), next_handle_(1), next_variable_handle_(0) {}
  ...
  StatusOr<GlobalDataHandle> ProduceAndRegsiterDAPPLEBufferHandle();

  StatusOr<DAPPLEBufferHandle> ResolveDAPPLEBufferHandle(
 ...
 private:
  ...
  int64 next_variable_handle_ TF_GUARDED_BY(mutex_);
  ...
  absl::flat_hash_map<int64, std::vector<std::unique_ptr<RegisteredBuffer>>>
      handle_to_registered_buffers_ TF_GUARDED_BY(mutex_);
