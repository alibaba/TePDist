...
StatusOr<llvm::Value*> ElementalIrEmitter::EmitFloatBinaryOp(
  ...
  switch (op->opcode()) {
    ...
    case HloOpcode::kDAPPLEAllReduce: {
      auto* dapple_inst = Cast<HloDAPPLEAllReduceInstruction>(op);
      auto reduction_type = dapple_inst->reduction_type();
      if (reduction_type == "Sum") {
        ...
        // N.B., HloOpcode::kMultiply not supported here.
        CHECK(0);
        return Unimplemented("binary floating point op '%s'",
                             HloOpcodeString(dapple_inst->opcode()));
      }
    }
    case HloOpcode::kAdd:
      return FAdd(lhs_value, rhs_value);
