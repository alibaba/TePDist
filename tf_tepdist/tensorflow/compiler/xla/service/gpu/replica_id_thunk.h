...
// Thunk that implements the ReplicaId HLO or PartitionId.
class ReplicaOrPartitionIdThunk : public Thunk {
 public:
  ReplicaOrPartitionIdThunk(Kind kind, const BufferAllocation::Slice& dest,
...
class ReplicaIdThunk : public ReplicaOrPartitionIdThunk {
 public:
  ReplicaIdThunk(const BufferAllocation::Slice& dest,
                 const HloInstruction* instr)
      : ReplicaOrPartitionIdThunk(Kind::kReplicaId, dest, instr) {}
};

class PartitionIdThunk : public ReplicaOrPartitionIdThunk {
 public:
  PartitionIdThunk(const BufferAllocation::Slice& dest,
                   const HloInstruction* instr)
      : ReplicaOrPartitionIdThunk(Kind::kPartitionId, dest, instr) {}
};
