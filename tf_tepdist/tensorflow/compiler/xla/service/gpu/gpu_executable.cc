...
Status GpuExecutable::ExecuteThunks(
  ...
  for (Thunk* thunk : thunk_schedule_->TotalOrder()) {
    ...
    if (thunk->hlo_instruction()->opcode() == HloOpcode::kDAPPLEAllReduce) {
    ...
    Thunk::ExecuteParams thunk_params{
        ...
        gpu_options && gpu_options->nccl_unique_id_callback()
            ...
            : nullptr,
        gpu_options && gpu_options->nccl_context()
            ? const_cast<NcclContext*>(gpu_options->nccl_context())
            : nullptr,
        &run_options->task_id(),
        run_options->comm_dev_mgr()};
