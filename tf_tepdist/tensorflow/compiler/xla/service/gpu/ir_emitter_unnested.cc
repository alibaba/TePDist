...
Status IrEmitterUnnested::HandlePartitionId(HloInstruction* hlo) {
  AddThunkToThunkSequence(
      absl::make_unique<PartitionIdThunk>(GetAllocationSlice(*hlo), hlo));
  return Status::OK();
}
...
Status IrEmitterUnnested::HandleDAPPLEAllReduce(HloInstruction* hlo) {
  const HloDAPPLEAllReduceInstruction* dapple_allreduce = 
      DynCast<HloDAPPLEAllReduceInstruction>(hlo);
  ...
  src_buffers.reserve(hlo->operand_count());
  dst_buffers.reserve(hlo->operand_count());
  ...
  for (int i = 0; i < hlo->operand_count(); ++i) {
    CHECK(hlo->operand(i)->shape().IsArray())
        << "Operands to all-reduce must be arrays: " << hlo->ToString();
    auto src_buffer = GetAllocationSlice(*hlo->operand(i));
    auto dst_buffer = GetAllocationSlice(
        *hlo, hlo->shape().IsTuple() ? ShapeIndex({i}) : ShapeIndex({}));
    src_buffers.push_back(src_buffer);
    dst_buffers.push_back(dst_buffer);
    element_counts.push_back(ShapeUtil::ElementsIn(hlo->operand(i)->shape()));
  }

  std::unique_ptr<KernelThunk> kernel_thunk;
#if 0
  if (2 == hlo->operand_count()) {
    // Only for creating local sum thunk.
    ElementalIrEmitter::HloToElementGeneratorMap operand_to_generator;
    for (const HloInstruction* operand : hlo->operands()) {
      operand_to_generator[operand] = [=](const llvm_ir::IrArray::Index& index) {
        auto s = GetIrArray(*operand, *hlo).EmitReadArrayElement(index, &b_);
        return s;
      };
    }
    GpuElementalIrEmitter elemental_emitter(hlo_module_config_, module_, &b_,
                                    GetNestedComputer());
    auto body_emitter = elemental_emitter
    ...
    // Thunk for local cross replica sum
    kernel_thunk = std::move(BuildKernelThunk(
        hlo, /*implements_whole_instruction=*/true, unroll_factor));
    Status emit_status =
        EmitTargetElementLoopInThunk(*hlo, body_emitter, kernel_thunk.get());
    CHECK(emit_status.ok());
  }
#endif

  // AllReduce Thunk for cross device reduction
  auto cross_replica_sum_thunk = absl::make_unique<DAPPLEAllReduceThunk>(
      std::move(src_buffers), std::move(dst_buffers), std::move(element_counts),
      /*replica_count=*/dapple_allreduce->num_replicas(),
      std::move(kernel_thunk), hlo);
  AddThunkToThunkSequence(std::move(cross_replica_sum_thunk));

  return Status::OK();
}

Status IrEmitterUnnested::HandleDAPPLEAllToAll(HloInstruction* hlo) {
  const HloDAPPLEAllToAllInstruction* dapple_alltoall = 
      DynCast<HloDAPPLEAllToAllInstruction>(hlo);
  ...
  auto dapple_alltoall_thunk = absl::make_unique<DAPPLEAllToAllThunk>(
      std::move(buffers), hlo);
  AddThunkToThunkSequence(std::move(dapple_alltoall_thunk));
  return Status::OK();
}

Status IrEmitterUnnested::HandleDAPPLEAllGather(HloInstruction* hlo) {
  const HloDAPPLEAllGatherInstruction* dapple_all_gather =
      DynCast<HloDAPPLEAllGatherInstruction>(hlo);

  std::vector<DAPPLEAllGatherThunk::Buffer> buffers;
  buffers.resize(hlo->operand_count());
  ...
  auto dapple_all_gather_thunk = absl::make_unique<DAPPLEAllGatherThunk>(
      std::move(buffers), hlo);
  AddThunkToThunkSequence(std::move(dapple_all_gather_thunk));
  return Status::OK();
}
