...
Status GpuCompiler::OptimizeHloModule(
  ...
  VLOG(2) << "[GpuCompiler] orig hlo module: " << hlo_module->ToString();

  {
...

    {
      ...
      // We comment this pass for some sharding notes are optimized.
  ...
  {
    // Automatic generation of parallelization specs
    HloPassPipeline pipeline("Pre-Opt Automatic Parallelization");
    pipeline.AddPass<HloDCE>();
    ...
    // TODO(b/64094172): make Call work on GPU instead of inlining.
    pipeline.AddPass<CallInliner>();
    pipeline.AddPass<HloLivenessOptimizer>();
 
    pipeline.AddPass<TupleSimplifier>();
    pipeline.AddPass<AutoParallel>(hlo_module->num_worker(), hlo_module->num_dev_per_worker());
    TF_RETURN_IF_ERROR(pipeline.Run(hlo_module).status());
  }
...

  {
    ...
    // We must postpone the algorithm selection pass until 
    // the HLO module has been transformed.
...

  {
    ...
    //HloPassPipeline scoped_fusion("scoped_fusion");
    //scoped_fusion.AddPass<ScopedFusion>();
    //scoped_fusion.AddPass<HloCSE>(/*is_layout_sensitive=*/true,
    //                              /*only_fusion_computations=*/true);
    //scoped_fusion.AddPass<HloDCE>();
    //TF_RETURN_IF_ERROR(scoped_fusion.Run(hlo_module).status());
  }
...
StatusOr<std::unique_ptr<HloModule>> GpuCompiler::FixHloModule(
    std::unique_ptr<HloModule> module, se::StreamExecutor* stream_exec,
    se::DeviceMemoryAllocator* device_allocator) {
  auto hlo_module = module.get();

  {
    HloPassPipeline pipeline("Auto par fix");
    //// Expand random number generation.
    pipeline.AddPass<RngExpander>();
    pipeline.AddPass<RngBitGeneratorExpander>(RandomAlgorithm::RNG_PHILOX);
...
StatusOr<std::unique_ptr<HloModule>> GpuCompiler::RunHloPasses(
  ...
  if (module->variable_map()->empty()) {
    // We must keep the module intact when additional computations are added.
...
StatusOr<std::unique_ptr<Executable>> GpuCompiler::RunBackend(
  ...
  if (!module->variable_map()->empty() ||
      (module->def_ctx() != nullptr && module->def_ctx()->name() != "Entry")) {
    TF_RETURN_IF_ERROR(PrepareHloModuleForIrEmitting(module.get()));
  }
  ...
  // Check if we need input_output_alias in current module.
  // Currently, only enabled in TF frontend.
  std::string frontend = ServiceEnv::frontend();
  auto& input_output_alias_map = module->def_ctx()->input_output_alias_map_;
  if (frontend == "TF-1.14" && !input_output_alias_map.empty()) {
    for (auto iter : input_output_alias_map) {
      CHECK(module->input_output_alias_config().SetUpAlias(
          {iter.second}/*output_index*/, iter.first/*param_number*/,
          {}/*param_index*/, HloInputOutputAliasConfig::kUserAlias).ok());
    }
  }
  ...
  bool dump_llvm_ptx = ServiceEnv::dump_llvm_ptx();

  std::string file_prefix = "test_" + module->name();
  if (dump_llvm_ptx) {
    std::ofstream llvm_file(file_prefix + "_llvm_module.txt");
    llvm_file << llvm_ir::DumpModuleToString(llvm_module);;
    llvm_file.close();
  }
  ...
  if (dump_llvm_ptx) {
    std::ofstream llvm_ptx_file(file_prefix + "_ptx_module.txt");
    llvm_ptx_file << backend_result.first;
    llvm_ptx_file.close();
  }
