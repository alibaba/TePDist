...
bool HloBufferIsReadOnly(const HloBuffer& buffer/*, 
                         const HloComputation* target_computation*/) {
  for (const HloValue* value : buffer.values()) {
    ...
    const bool is_entry_parameter =
        ...
        //instruction->parent() == target_computation
...
bool BufferAssigner::MaybeAssignBuffer(BufferAllocation* allocation,
  ...
  // We do not share the memory allocation for the operands whose names are different.
  for (auto* hlo_vals : hlo_buffer.values()) {
    for (auto& hlo_use : hlo_vals->uses()) {
      auto* user = hlo_use.instruction;
      for (auto* op : user->operands()) {
        if (op == user) continue;
        for (const auto& operand_slice : assignment->GetAllSlices(op, /*index=*/{})) {
          BufferAllocation* op_allocation =
              assignment->GetMutableAllocation(operand_slice.index());
          if (op_allocation == allocation) return false;
        }
      }
    }
  }
...
Status BufferAssigner::MergeInplaceOpBuffers(BufferAssignment* assignment) {
  ...
  for (HloComputation* computation : assignment->module().computations()) {
    for (HloInstruction* instruction : computation->instructions()) {
      ...
      // read-only.
      if (HloBufferIsReadOnly(operand_buffer/*, assignment->module().entry_computation()*/)) {
        continue;
      }
  ...
  for (const HloValue* value : hlo_buffer->values()) {
    ...
    const bool is_entry_parameter =
        ...
        instruction->parent() ==
            ...
            //assignment->module().entry_computation();
