...
template <typename T>
struct VirtualClientAndPtr {
  VirtualClientAndPtr() = default;
  // pybind11 requires that we define a constructor that takes a raw pointer,
  // but it should be unreachable.
  explicit VirtualClientAndPtr(T*) {
    LOG(FATAL) << "VirtualClientAndPtr should constructed via WrapWithClient.";
  }

  VirtualClientAndPtr(const VirtualClientAndPtr&) = default;
  VirtualClientAndPtr(VirtualClientAndPtr&&) = default;
  VirtualClientAndPtr& operator=(const VirtualClientAndPtr&) = default;
  VirtualClientAndPtr& operator=(VirtualClientAndPtr&&) = default;
...
template <typename T>
VirtualClientAndPtr<T> WrapWithClient(std::shared_ptr<VirtualClient> client,
                               T* contents) {
  VirtualClientAndPtr<T> result;
  result.client = std::move(client);
  result.contents = contents;
  return result;
}
...
// A pair of a PjRtClient reference and an owned pointer to T.
template <typename T>
struct VirtualClientAndUniquePtr {
  VirtualClientAndUniquePtr() = default;
  // pybind11 requires that we define a constructor that takes a raw pointer,
  // but it should be unreachable.
  explicit VirtualClientAndUniquePtr(T*) {
    LOG(FATAL) 
        << "VirtualClientAndUniquePtr should constructed via WrapWithClient.";
  }
  VirtualClientAndUniquePtr(const VirtualClientAndUniquePtr&) = delete;
  VirtualClientAndUniquePtr(VirtualClientAndUniquePtr&&) = default;
  VirtualClientAndUniquePtr& operator=(const VirtualClientAndUniquePtr&) = delete;
  VirtualClientAndUniquePtr& operator=(VirtualClientAndUniquePtr&&) = default;
...
template <typename T>
VirtualClientAndUniquePtr<T> WrapWithClient(std::shared_ptr<VirtualClient> client,
                                     std::unique_ptr<T> contents) {
  VirtualClientAndUniquePtr<T> result;
  result.client = std::move(client);
  result.contents = std::move(contents);
  return result;
}
...
PYBIND11_DECLARE_HOLDER_TYPE(T, xla::VirtualClientAndPtr<T>);
PYBIND11_DECLARE_HOLDER_TYPE(T, xla::VirtualClientAndUniquePtr<T>);
