...
StatusOr<std::shared_ptr<TrackedDeviceBuffer>> AllocateGPUBuffer(
    const Shape& on_host_shape, Device* device, LocalDeviceState* local_device,
    se::Stream* copy_stream, PjRtClient* client) {
  if (on_host_shape.IsTuple() && on_host_shape.tuple_shapes_size() == 0) {
    return InvalidArgument("Can't make a buffer from an empty tuple");
  ...
  Shape on_device_shape = dst_buffer.on_device_shape();
  if (on_device_shape.IsTuple()) {
    return InvalidArgument("on_device_shape should not be tuple.");
  }
  ...
  return std::move(dst_device_buffer);
}
...
StatusOr<std::unique_ptr<PjRtBuffer>> PjRtBuffer::FromHostBuffer(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      pjrt_utils::AllocateDestinationBuffer(compact_shape, device, local_device,
...
/*static*/ void PjRtBuffer::MakeCrossHostReceiveBuffers(
  ...
  for (const auto& shape : shapes) {
    StatusOr<std::unique_ptr<PjRtBuffer>> buffer_or =
        pjrt_utils::AllocateDestinationBuffer(shape, device, local_device,
...
Status PjRtBuffer::CopyToHostAsync() {
  ...
  if (!event_or.ok()) {
    ...
    pjrt_utils::StallStreamOnError(local_device, stream);
    return event_or.status();
  }
  ...
  pjrt_utils::RecordUsage(std::move(device_buffer), local_device,
                          local_device, usage_event, stream,
...
PjRtBuffer::CopyToDeviceHelper(
  ...
  TF_ASSIGN_OR_RETURN(
      std::unique_ptr<PjRtBuffer> py_buffer,
      pjrt_utils::AllocateDestinationBuffer(on_host_shape_, dst_device,
                                            dst_local_device,
  ...
  StatusOr<std::shared_ptr<BufferSequencingEvent>> copy_event_or =
    ...
    TF_RETURN_IF_ERROR(pjrt_utils::AddDestinationBufferSynchronization(
        transfer_local_device, std::move(dst_device_buffer), event,
        transfer_stream));
    return event;
  }();
  if (!copy_event_or.ok()) {
    pjrt_utils::StallStreamOnError(transfer_local_device, transfer_stream);
...
StatusOr<std::unique_ptr<PjRtBuffer>> PjRtBuffer::CopyToDevice(
  ...
  pjrt_utils::RecordUsage(std::move(src_device_buffer),
                          device_->local_device_state(),
...
std::unique_ptr<PjRtBuffer> OutputBufferHelper(
  ...
  pjrt_utils::RecordUsage(py_buffer->GetBufferWithUsageHold(), local_device,
                          local_device, definition_event,
                          local_device->compute_stream(),
                          /*prefer_to_retain_reference=*/false);
  return py_buffer;
}

Device* LookupDevice(const PjRtClient& client, int device_id) {
...
StatusOr<ScopedShapedBuffer> PjRtExecutable::EnqueueExecution(
  ...
  VLOG(2) << "Replica " << replica << ", partition " << partition
...
PjRtExecutable::ExecuteHelper(absl::Span<PjRtBuffer* const> argument_handles,
  ...
  Device* device = pjrt_utils::LookupDevice(*client_, device_id);
  ...
  if (options.untuple_result && result_buffer.on_host_shape().IsTuple()) {
    ...
    for (int i = 0; i < tuple_count; ++i) {
      ...
      outputs.push_back(pjrt_utils::OutputBufferHelper(
          &tuple_buffer, definition_event, client_, device, device_state));
    }
    if (device_state->allocation_model() == LocalDeviceState::kSynchronous) {
    ...
    outputs.push_back(pjrt_utils::OutputBufferHelper(
        &result_buffer, definition_event, client_, device, device_state));
  }
  ...
  for (PjRtBuffer::ScopedHold& b : device_buffers) {
    ...
    if (b.type() == PjRtBuffer::ScopedHold::kUsage) {
      pjrt_utils::RecordUsage(std::move(b), device_state, device_state,
...
/*static*/ StatusOr<std::unique_ptr<PjRtExecutable>> PjRtExecutable::Compile(
  ...
  for (int replica = 0; replica < num_replicas; ++replica) {
    for (int partition = 0; partition < num_partitions; ++partition) {
      ...
      Device* device = pjrt_utils::LookupDevice(*client, device_id);
