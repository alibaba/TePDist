...
Status BuildDistributedDevicesForDistHlo(
    std::vector<std::unique_ptr<LocalDeviceState>> local_device_states,
    int node_id, std::vector<std::unique_ptr<Device>>* devices,
    GpuExecutableRunOptions* gpu_executable_run_options) {
  // Map of local device_id to global_device_id
  auto num_local_devs = local_device_states.size();
  std::vector<GlobalDeviceId> gpu_device_ids(num_local_devs);
  for (auto& local_device : local_device_states) {
    int local_device_ordinal = local_device->device_ordinal();
    auto global_dev_id = node_id * num_local_devs + local_device_ordinal;
    gpu_device_ids[local_device_ordinal] = GlobalDeviceId(global_dev_id);
    const se::DeviceDescription& description =
        local_device->executor()->GetDeviceDescription();
    auto device = absl::make_unique<GpuDevice>(
        local_device_ordinal, std::move(local_device), description.name(),
        /*node_id=*/node_id);
    devices->push_back(std::move(device));
  }
...
StatusOr<std::shared_ptr<PjRtClient>> GetNvidiaGpuClient(
  ...
  BuildDistributedDevicesForDistHlo(std::move(local_device_states),
