...
class PjRtBuffer {
 public:
  ...
  //static StatusOr<std::unique_ptr<PjRtBuffer>> FromHostBuffer(
  //    const void* data, const Shape& shape, bool force_copy,
  //    std::shared_ptr<void> buffer_reference, VirtualClient* client);
  ...
  TrackedDeviceBuffer* device_buffer() { return device_buffer_.get(); }

  // Clear the cached value of the buffer on the host.
  //
  // When should be called: the device_buffer_ is refreshed while the
  // host_value_ is already cached for previous stale device_buffer_.
  // New incoming call of `CopyToHostAsync()` should generate new host_value_
  // instead of taking the cached stale one.
  void ClearHostCache() { host_value_ = nullptr; }
...
struct ExecuteOptions {
  ...
  int64 global_step = -1;
  // If true, the save procedure happens during one ExecuteRPCPlan call before ExecuteTask.
  // This is useful when client call saving but DAPPLEBuffers are not initialized. For example,
  // the saver_hooks.begin() calls saving before running training computation, which even the
  // strategy of HLO are not ready.
  bool lazy_save_ckpt = false;

  // If true, the server restores model parameters from checkpoint file.
...
// The following functions are exposed outside for reusing purpose.
