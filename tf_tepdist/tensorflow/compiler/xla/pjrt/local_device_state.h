...
class LocalDeviceState {
 public:
  ...
  se::Stream* send_stream() const { return send_stream_.get(); }
  se::Stream* recv_stream() const { return recv_stream_.get(); }
 ...
 private:
  ...
  std::unique_ptr<se::Stream> send_stream_;
  std::unique_ptr<se::Stream> recv_stream_;
