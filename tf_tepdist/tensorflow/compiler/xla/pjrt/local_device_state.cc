...
LocalDeviceState::LocalDeviceState(se::StreamExecutor* executor,
  ...
  send_stream_ = absl::make_unique<se::Stream>(executor);
  recv_stream_ = absl::make_unique<se::Stream>(executor);
...
Status LocalDeviceState::SynchronizeAllActivity() {
  ...
  status.Update(send_stream_->BlockHostUntilDone());
  status.Update(recv_stream_->BlockHostUntilDone());
