...
int RealMain(int argc, char** argv) {
  ...
  std::vector<tensorflow::Flag> flag_list = {
      ...
      tensorflow::Flag(
          "ip", &ip_addr, "ip_address of where current service locates."),
      tensorflow::Flag("port", &port, "The TCP port to listen on"),
      tensorflow::Flag("task_index", &task_index, "task index"),
      tensorflow::Flag(
          "any", &any_address,
  ...
  std::unique_ptr<xla::GRPCService> service =
      xla::GRPCService::NewService(platform, task_index).ConsumeValueOrDie();

  ::grpc::ServerBuilder builder;
  string server_address(
      absl::StrFormat("%s:%d", any_address ? "[::]" : ip_addr, port));
