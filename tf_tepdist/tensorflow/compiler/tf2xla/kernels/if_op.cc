...
class ComputationScopeContext {
 public:
  explicit ComputationScopeContext(XlaCompiler* compiler)
      : compiler_(compiler), stacked_is_entry_(compiler->is_entry()) {
    compiler_->set_is_entry(false);
  }

  ~ComputationScopeContext() {
    compiler_->set_is_entry(stacked_is_entry_);
  }

 private:
  XlaCompiler* compiler_;
  const bool stacked_is_entry_;
};
...
void XlaIfOp::Compile(XlaOpKernelContext* ctx) {
  ...
  ComputationScopeContext scope_context(compiler);
