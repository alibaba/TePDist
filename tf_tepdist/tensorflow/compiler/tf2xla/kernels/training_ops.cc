...
class ResourceApplyAdamWeightDecay : public XlaOpKernel {
 public:
  explicit ResourceApplyAdamWeightDecay(
  ...
  void Compile(XlaOpKernelContext* ctx) override {
    ...
    TensorShape epsilon_shape = ctx->InputShape(8);
    TensorShape grad_shape = ctx->InputShape(9);
    TensorShape weight_decay_ratio_shape = ctx->InputShape(10);
    ...
    OP_REQUIRES(ctx, TensorShapeUtils::IsScalar(weight_decay_ratio_shape),
                errors::InvalidArgument("weight_decay_ratio is not a scalar : ",
                                        weight_decay_ratio_shape.DebugString()));
    ...
    xla::XlaOp epsilon = ctx->Input(8);
    xla::XlaOp grad = ctx->Input(9);
    xla::XlaOp weight_decay_ratio = ctx->Input(10);

    // alpha <- learning_rate * sqrt(1 - beta2^t) / (1 - beta1^t)
    // m_t <- beta1 * m_{t-1} + (1 - beta1) * g_t
    // v_t <- beta2 * v_{t-1} + (1 - beta2) * g_t * g_t
    // variable <- variable -
    //    alpha * (m_t / (sqrt(v_t) + epsilon) + w_d * variable)
    ...
    if (use_weight_decay_) {
      var = var - alpha * (
          m  / (xla::Sqrt(v) + epsilon) + weight_decay_ratio * var);
    } else {
...
REGISTER_XLA_OP(
    Name("ResourceApplyAdamWeightDecay").TypeConstraint("T", kFloatTypes),
    ResourceApplyAdamWeightDecay);
