...
// Records the arg index from _XlaRun Op to _XlaCompile Op
xla::StatusOr<std::unordered_map<int, int>>
RecordArgsMappingFromRealArgsToCompileOp(const Graph& graph) {
  std::unordered_map<int, int> args_indices_map;
  for (const Node* n : graph.nodes()) {
    if (n->IsArg() && HasNodeAttr(n->def(), "run_index")) {
      int index_in_compile_op;
      int index_in_run_op;
      TF_RETURN_IF_ERROR(GetNodeAttr(n->attrs(), "index", &index_in_compile_op));
      TF_RETURN_IF_ERROR(GetNodeAttr(n->attrs(), "run_index", &index_in_run_op));
      TF_RET_CHECK(index_in_compile_op >= 0) << "Negative _Arg index for CompileOp";
      TF_RET_CHECK(index_in_run_op >= 0) << "Negative _Arg index for real args";
      args_indices_map[index_in_run_op] = index_in_compile_op;
    }
  }
  return args_indices_map;
}
...
Status BuildComputation(
    ...
    bool always_return_tuple, bool use_tuple_arg,
    bool alias_resource_update,  // tf2 added is_entry_computation / alias_resource_update
  ...
  *num_nonconst_outputs = elems.size();
  ...
  for (const XlaResource* resource : arg_resources) {
    ...
    if (return_updated_values_for_all_resources || modified) {
      retval_metadata.set_op_name(arg.name);
      builder->SetOpMetadata(retval_metadata);
  ...
  *num_computation_outputs = elems.size();
  ...
  xla::StatusOr<xla::XlaComputation> computation_status =
      builder->Build(var_map, init_specs_map, fetch_vars_list);
...
// Auto Parallel Begin
Status XlaCompiler::BuildVariablesRuntimeInfo(
    const std::vector<XlaCompiler::Argument>& args,
    std::map<int, string> *variable_map, std::map<int, string> *init_specs_map,
    std::vector<int>* fetch_vars_list) {
  int count = 0;
  ...
  for (std::vector<XlaCompiler::Argument>::size_type i = 0; i < args.size();
    ...
    switch (arg.kind) {
      case XlaCompiler::Argument::kResource: {
        ...
        TF_RET_CHECK(absl::holds_alternative<TensorShape>(arg.shape));
        (*variable_map)[count] = arg.name;
        (*init_specs_map)[count] = arg.rng_initialization_bytes;
        if (arg.need_fetch) fetch_vars_list->push_back(count);
        if (arg.initialized) {
          ++count;
        }
        break;
...
void XlaCompiler::set_is_entry(bool is_entry) {
  is_entry_ = is_entry;
}
...
Status XlaCompiler::BuildArguments(
  ...
  TF_ASSIGN_OR_RETURN(auto real_arg_indices_map,
                      RecordArgsMappingFromRealArgsToCompileOp(graph));
  ...
  // Auto Parallel Begin
  std::string var_map_file_name;
  std::vector<std::string> args_name;
  std::vector<bool> backwards;
  std::vector<int> op_groups;
  std::ofstream out_var_map_file;
  if (is_entry_) {
    CHECK(ReadStringFromEnvVar("VARIABLE_MAP_FILE_PATH", "./variable_map.txt",
                               &var_map_file_name).ok());
    VLOG(2) << "var_map_file_name = " << var_map_file_name;
    out_var_map_file.open(var_map_file_name);
  }
  // Auto Parallel End
  ...
  for (std::vector<XlaCompiler::Argument>::size_type i = 0; i < args.size();
    ...
    switch (arg.kind) {
      case XlaCompiler::Argument::kResource: {
        ...
        // Auto Parallel Begin
        if (is_entry_) {
          auto shape = absl::get<TensorShape>(arg.shape);
          VLOG(2) << "[VARIABLE MAP]" << ":" << input_to_args->size()
                  << ":" << arg.name
                  << ", " << shape.DebugString();
          CHECK(out_var_map_file.is_open());
          out_var_map_file << input_to_args->size() << ":" << arg.name
                           << ", " << shape.DebugString()
                           << std::endl;
        }
        // Auto Parallel End
        if (arg.initialized) {
          args_name.push_back(arg.name);
          backwards.push_back(arg.backward);
          op_groups.push_back(arg.op_group);
  ...
  if (use_tuple_arg) {
    ...
    for (std::vector<int>::size_type i = 0; i < input_to_args->size(); ++i) {
      int arg_idx = (*input_to_args)[i];
      auto arg_iter = real_arg_indices_map.find(arg_idx);
      int real_idx =
          arg_iter == real_arg_indices_map.end() ? arg_idx : arg_iter->second;
      auto it = arg_shardings.find(real_idx);
      xla::XlaScopedShardingAssignment assign_sharding(
          builder, it == arg_shardings.end() ? absl::optional<xla::OpSharding>()
                                             : it->second);
      
      if (!args_name[i].empty()) {
        arg_metadata.set_op_name(args_name[i]);
        arg_metadata.set_backward(backwards[i]);
        arg_metadata.set_op_group(op_groups[i]);
        builder->SetOpMetadata(arg_metadata);
      }
...
Status XlaCompiler::CompileGraph(
  ...
  // Auto Parallel Begin
  if (is_entry_) {
    TF_RETURN_IF_ERROR(BuildVariablesRuntimeInfo(
        real_args, &result->variable_map, &result->init_specs_map,
        &result->fetch_vars_list));
  }
  // Auto Parallel End
  std::vector<XlaExpression> arg_expressions;
  ...
  TF_RETURN_IF_ERROR(BuildComputation(
      ...
      result->variable_map, result->init_specs_map,
      result->fetch_vars_list));
