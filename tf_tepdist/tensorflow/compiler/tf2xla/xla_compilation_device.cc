...
xla::StatusOr<bool> ParseBackwardFlagFromNodeDef(const NodeDef& node_def) {
  if (!HasNodeAttr(node_def, kBackward)) {
    return false;
  }
  bool value;
  TF_RETURN_IF_ERROR(GetNodeAttr(node_def, kBackward, &value));
  return value;
}

xla::StatusOr<int> ParseOpGroupFromNodeDef(const NodeDef& node_def) {
  if (!HasNodeAttr(node_def, kOpGroup)) {
    return 0;
  }
  int value;
  TF_RETURN_IF_ERROR(GetNodeAttr(node_def, kOpGroup, &value));
  return value;
}
...
void XlaCompilationDevice::Compute(OpKernel* op_kernel,
  ...
  auto* b = XlaContext::Get(context).builder();
	...
	auto backward_result = ParseBackwardFlagFromNodeDef(op_kernel->def());
  OP_REQUIRES_OK(context, backward_result.status());
  bool backward = backward_result.ValueOrDie();

	auto op_group_result = ParseOpGroupFromNodeDef(op_kernel->def());
  OP_REQUIRES_OK(context, op_group_result.status());
  int op_group = op_group_result.ValueOrDie();

  xla::OpMetadata metadata;
  metadata.set_op_type(op_kernel->type_string());
  metadata.set_op_name(op_kernel->name());
  metadata.set_backward(backward);
  metadata.set_op_group(op_group);
