...
// Auto Parallel Begin
static bool FuncLibaryContainsUpdateOp(
    Graph* graph, std::string resource_apply, std::string v1_apply) {
  const GraphDef& graph_def = graph->ToGraphDefDebug();
  if (graph_def.has_library()) {
    for (const FunctionDef& func_def : graph_def.library().function()) {
      for (const NodeDef& n_def : func_def.node_def()) {
        if (n_def.op() == resource_apply || n_def.op() == v1_apply) return true; 
      }
    }
  }
  return false;
...
static bool AdamTrainGraph(Graph* graph) {
  for (Node* node : graph->op_nodes()) {
    if (node->type_string() == "ApplyAdam" ||
        node->type_string() == "ResourceApplyAdam") return true;
  }
  return FuncLibaryContainsUpdateOp(
      graph, "ApplyAdam", "ResourceApplyAdam");
}

static bool AdamWTrainGraph(Graph* graph) {
  for (Node* node : graph->op_nodes()) {
    if (node->type_string() == "ApplyAdamWeightDecay" ||
        node->type_string() == "ResourceApplyAdamWeightDecay") return true;
  }
  return FuncLibaryContainsUpdateOp(
      graph, "ApplyAdamWeightDecay", "ResourceApplyAdamWeightDecay");
}

static bool AdagradTrainGraph(Graph* graph) {
  for (Node* node : graph->op_nodes()) {
    if (node->type_string() == "ApplyAdagrad" ||
        node->type_string() == "ResourceApplyAdagrad") return true;
  }
  return FuncLibaryContainsUpdateOp(
      graph, "ApplyAdagrad", "ResourceApplyAdagrad");
}

static bool AdadeltaTrainGraph(Graph* graph) {
  for (Node* node : graph->op_nodes()) {
    if (node->type_string() == "ApplyAdadelta" ||
        node->type_string() == "ResourceApplyAdadelta") return true;
  }
  return FuncLibaryContainsUpdateOp(
      graph, "ApplyAdadelta", "ResourceApplyAdadelta");
}

static bool GDTrainGraph(Graph* graph) {
  for (Node* node : graph->op_nodes()) {
    if (node->type_string() == "GradientDescent" ||
        node->type_string() == "ResourceApplyGradientDescent") return true;
  }
  return FuncLibaryContainsUpdateOp(
      graph, "GradientDescent", "ResourceApplyGradientDescent");
}

static bool RMSPropTrainGraph(Graph* graph) {
  for (Node* node : graph->op_nodes()) {
    if (node->type_string() == "RMSProp" ||
        node->type_string() == "ResourceApplyRMSProp") return true;
  }
  return FuncLibaryContainsUpdateOp(
      graph, "RMSProp", "ResourceApplyRMSProp");
}

static bool VarMomTrainGraph(Graph* graph) {
  bool momentum = false, variable_v2 = false;
  for (Node* node : graph->op_nodes()) {
    if (!variable_v2 && node->type_string() == "VariableV2") {
      variable_v2 = true;
    }

    if (!momentum && 
        (node->type_string() == "ResourceApplyMomentum" ||
         node->type_string() == "ApplyMomentum")) {
      momentum = true;
    }

    if (variable_v2 && momentum) return true;
  }
  return FuncLibaryContainsUpdateOp(
      graph, "ResourceApplyMomentum", "ApplyMomentum");
}

static bool IsTrainingGraph(Graph* graph) {
  if (AdamTrainGraph(graph) ||
      AdamWTrainGraph(graph) ||
      AdagradTrainGraph(graph) ||
      AdadeltaTrainGraph(graph) ||
      GDTrainGraph(graph) ||
      RMSPropTrainGraph(graph) ||
      VarMomTrainGraph(graph)) {
    return true;
  }
...
class MarkForCompilationPassImpl {
 ...
 private:
  // Auto Parallel Begin
  Status RunVarsConsumptionsColocation();
  // Auto Parallel End
...
Status MarkForCompilationPassImpl::Run() {
  ...
  // Auto Parallel Begin
  // Only training graph (the graph contains ApplyXXX or ResourceApplyXXX)
  // should be compiled.
  if (!IsTrainingGraph(graph_)) return Status::OK();
  ...
  // Auto Parallel Begin
  TF_RETURN_IF_ERROR(RunVarsConsumptionsColocation());
  // Auto Parallel End
...
// Auto Parallel Begin
Status MarkForCompilationPassImpl::RunVarsConsumptionsColocation() {
  Cluster* training_cluster = nullptr;
  std::unordered_set<Node*> read_var_ops;
  std::unordered_set<Cluster*> clusters_set;
  bool use_cond_v2 = false;
  for (Node* n : compilation_candidates_) {
    Cluster* cluster = GetClusterForNode(n);
    clusters_set.insert(cluster);
    if (n->type_string() == "ApplyAdam" ||
        n->type_string() == "ResourceApplyAdam" ||
        n->type_string() == "ApplyAdamWeightDecay" ||
        n->type_string() == "ResourceApplyAdamWeightDecay" ||
        n->type_string() == "GradientDescent" ||
        n->type_string() == "ApplyAdagrad" ||
        n->type_string() == "ResourceApplyAdagrad" ||
        n->type_string() == "ApplyAdadelta" ||
        n->type_string() == "ResourceApplyAdadelta" ||
        n->type_string() == "ResourceApplyGradientDescent" ||
        n->type_string() == "RMSProp" ||
        n->type_string() == "ResourceApplyRMSProp" ||
        n->type_string() == "ResourceApplyMomentum" ||
        n->type_string() == "ApplyMomentum") {
      training_cluster = cluster;
    } else if (n->type_string() == "ReadVariableOp" ||
               n->type_string() == "ResourceGather") {
      CHECK(read_var_ops.insert(n).second);
  ...
  if (use_cond_v2) {
    std::vector<Cluster*> clusters_list(clusters_set.begin(), clusters_set.end());
    while (clusters_list.size() > 1) {
      Cluster* from_cluster = clusters_list.back();
      clusters_list.pop_back();
      Cluster* dst_cluster = clusters_list.back();
      from_cluster->Merge(dst_cluster);
      int from_id = from_cluster->cycles_graph_node_id();
      int dst_id = dst_cluster->cycles_graph_node_id();
      cluster_for_node_[from_id].Merge(&cluster_for_node_[dst_id]);
    }

    std::unordered_set<Cluster*> final_set;
    for (Node* n : compilation_candidates_) {
      Cluster* cluster = GetClusterForNode(n);
      final_set.insert(cluster);
    }
    VLOG(1) << "cluster size = " << final_set.size();
  } else {
    CHECK(training_cluster);
    std::unordered_set<Cluster*> other_clusters;
    std::deque<Node*> worklist;
    std::unordered_set<Node*> visited;
    for (auto src_node : read_var_ops) {
      std::deque<Node*> worklist;
      std::unordered_set<Node*> visited;
      worklist.push_back(src_node);
      ...
      while (!worklist.empty()) {
        ...
        auto cluster = GetClusterForNode(node);
        if (!other_clusters.count(cluster) && cluster && cluster != training_cluster) {
          CHECK(other_clusters.insert(cluster).second);
        }
        for (auto e : node->out_edges()) {
          //if (e->IsControlEdge()) continue;

          auto tgt = e->dst();
          if (visited.count(tgt)) continue;
          if (tgt->type_string() == "Snapshot" ||
              tgt->type_string() == "AssignVariableOp") continue;
    ...
    VLOG(1) << "[DBUG] other_clusters size = " << other_clusters.size();
    //for (auto tgt_cluster : other_clusters) {
    //  CHECK(MergeClusters(training_cluster, tgt_cluster));
    for (auto tgt_cluster : other_clusters) {
      // Merge the clusters.
      training_cluster->Merge(tgt_cluster);

      // Merge the UnionFind<Cluster*>.
      int from = training_cluster->cycles_graph_node_id();
      int to = tgt_cluster->cycles_graph_node_id();
      cluster_for_node_[from].Merge(&cluster_for_node_[to]);
    }
  }
  return Status::OK();
