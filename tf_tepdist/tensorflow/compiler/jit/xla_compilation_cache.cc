...
Status XlaCompilationCache::BuildExecutable(
  ...
  if (rpc_client_) {
    VLOG(0) << "Start GPRC client request...";
    auto compile_result =
        rpc_client_->BuildExecutionPlan(*result.computation, build_options);
    CHECK(compile_result.ok()) << "Remote compilation failed!";
    *execution_handle = compile_result.ValueOrDie();
    *executable = nullptr;
  } else {
      TF_ASSIGN_OR_RETURN(
        auto executables,
...
Status XlaCompilationCache::Compile(
  ...
  return CompileImpl(options, function, args, compile_fn,
                     ...
                     out_compilation_result, out_executable, handle);
}
...
Status XlaCompilationCache::CompileImpl(
    ...
    const XlaCompiler::CompilationResult** out_compilation_result,
    xla::LocalExecutable** out_executable,
    xla::ExecutionPlanHandle* out_handle) {
  ...
  if (!entry->compiled) {
    ...
    entry->compilation_status =
        BuildExecutable(options, entry->compilation_result, &entry->executable,&entry->handle);
  ...
  *out_executable = entry->executable.get();
  if (out_handle) {
    *out_handle = entry->handle;
    VLOG(2) << "[Client] Get execution handle from service:"
            << out_handle->handle();
  } else {
    VLOG(2) << "Entering LocalClient & LocalService mode.";
  }
  return Status::OK();
}
