...
class XlaCompilationCache : public ResourceBase {
 ...
 private:
  ...
  Status CompileImpl(
      ...
      const XlaCompiler::CompilationResult** out_compilation_result,
      xla::LocalExecutable** out_executable,
      xla::ExecutionPlanHandle* out_handle=nullptr);
  ...
  // GRPC client (owned by ClientLibrary)
  xla::Client* const rpc_client_ = nullptr;
  ...
  struct Entry {
    ...
    // Handle given to a user that represents an execution that the user
    // launched asynchronously on the device
    xla::ExecutionPlanHandle handle GUARDED_BY(mu);
  };
