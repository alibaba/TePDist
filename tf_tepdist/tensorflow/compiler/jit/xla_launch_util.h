...
struct OptionalTensor {
  ...
  string rng_initialization_bytes;
  TensorShape initialization_shape;
...
class XlaComputationLaunchContext {
 public:
  ...
  //
  // When in Client/Server mode, we set applying_variable_updates=false to
  // disable fetching and updating the latest variables from the server every
  // training iteration for better performance.
  Status PopulateOutputs(
      ...
      const std::unordered_set<int>* vars_update_set = nullptr);
  
  // Return the argument list. Only valid after PopulateInputs() has been
  // called.
  const std::vector<xla::ShapedBuffer*>& arguments() const { return arg_ptrs_; }

  // Return trainable variable argument list. Only valid after
  // SeparateArguments() has been called.
  const std::vector<xla::ShapedBuffer*>& var_arguments() const {
      return variable_ptrs_;
  }

  void set_fetch_all_variables(bool fetch_all) { fetch_all_ = fetch_all; }
  const bool fetch_all() const { return fetch_all_; }
  const std::unordered_set<int>& vars_update_set() const { return vars_update_set_; }

  // Return input data argument list. Only valid after
  // SeparateArguments() has been called.
  const std::vector<xla::ShapedBuffer*>& input_arguments() const {
      return input_data_ptrs_;
  }

  const bool init_from_client() const { return init_from_client_; }
 ...
 private:
  ...
  bool init_from_client_ = true;
  bool fetch_all_ = false;
  std::vector<xla::ShapedBuffer*> variable_ptrs_;
  std::vector<xla::ShapedBuffer*> input_data_ptrs_;
  std::unordered_set<int> vars_update_set_;
};
