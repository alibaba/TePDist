...
Status SnapshotResourceVariables(OpKernelContext* ctx,
  ...
  for (int i = 0; i < variable_indices.size(); i++) {
    if (variable_infos[i].var()) {
      ...
      tensor.rng_initialization_bytes = \
          *variable_infos[i].var()->mutable_initializer_string();
      tensor.initialization_shape = \
          *variable_infos[i].var()->mutable_initializer_shape();
      tensor.need_fetch = variable_infos[i].var()->need_fetch();
      tensor.backward = variable_infos[i].var()->backward();
      tensor.op_group = variable_infos[i].var()->op_group();
    } else {
...
void XlaComputationLaunchContext::SeparateArguments(
    const XlaCompiler::CompilationResult* kernel,
    const std::map<int, OptionalTensor>& variables) {
  for (int i = 0; i < kernel->xla_input_shapes.size(); ++i) {
    int arg_num = kernel->input_mapping[i];
    if (variables.count(arg_num)) {
      auto& var = variables.at(arg_num);
      init_from_client_ &= (var.value.tensor_data().data() != nullptr);
      if (var.need_fetch || fetch_all_) {
        vars_update_set_.insert(variable_ptrs_.size());
      }
      variable_ptrs_.push_back(arg_ptrs_[i]);
    } else {
      input_data_ptrs_.push_back(arg_ptrs_[i]);
    }
  }
}
...
Status XlaComputationLaunchContext::PopulateOutputs(
    ...
    const xla::HloInputOutputAliasConfig& input_output_alias,
    const std::map<int, OptionalTensor>& resource_var_snapshots,
    const std::unordered_set<int>* vars_update_set) {
  se::Stream* stream =
      ctx->op_device_context() ? ctx->op_device_context()->stream() : nullptr;
  ...
  // Applying variable updates every iteration is disabled.
  if (vars_update_set && vars_update_set->empty()) return Status::OK();
  ...
  for (int i = 0; i < kernel->resource_updates.size(); ++i) {
    if (vars_update_set && !vars_update_set->count(i)) continue;
  ...
  TF_RETURN_IF_ERROR(LockVariables(absl::MakeSpan(variable_infos)));
  int var_info_index = 0;
  for (int i = 0; i < kernel->resource_updates.size(); ++i) {
    if (vars_update_set && !vars_update_set->count(i)) continue;
    Allocator* allocator = ctx->device()->GetAllocator({});
    const XlaCompiler::ResourceUpdate& write = kernel->resource_updates[i];

    if (variable_infos[var_info_index].var()->tensor()->dtype() != write.type) {
      return errors::Internal("Mismatched type in variable write");
    }

    if (allocate_xla_tensors_) {
      TF_RETURN_IF_ERROR(SetBufferForResourceVarTensorUnderAllocateXlaTensors(
          input_output_alias, output_num, ctx, var_info_index, write, &output,
      ...
      *variable_infos[var_info_index].var()->tensor() = output_tensor;
      variable_infos[var_info_index].var()->is_initialized |= write.modified;
    }
    ++var_info_index;
    ++output_num;
  }
...
Status XlaComputationLaunchContext::BuildXlaCompilerArguments(
  ...
  for (int64 input_num = 0; input_num < ctx->num_inputs(); ++input_num) {
    ...
    if (constant_args.count(input_num) > 0) {
      ...
      arg.need_fetch = variable.need_fetch;
      arg.op_group = variable.op_group;
      arg.backward = variable.backward;
      if (variable.present) {
        const Tensor& value = variable.value;
        arg.type = value.dtype();
        if (value.tensor_data().data()) {
          arg.shape = value.shape();
        } else {
          arg.shape = variable.initialization_shape;
        }
        arg.rng_initialization_bytes = variable.rng_initialization_bytes;
